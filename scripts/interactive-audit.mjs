...
try {
  ...
  await page
    .waitForSelector(
      '[data-testid="balance-total"], [data-testid="header-settings-btn"], [data-testid="header-theme-toggle"]',
      { timeout: 10000 },
    )
    .catch(() => {});
  ...
  if (await themeToggle.isVisible()) {
    console.log(
      'Quick Theme Toggle found. Initial active theme:',
      await page.evaluate(() =>
  ...
  const settingsBtn = page
    .locator('[data-testid="wallet-menu"]')
    .or(page.locator('[data-testid="header-settings-btn"]'));
  ...
  const receiveBtn = page
    .locator('button:has-text("Receive")')
    .or(page.locator('[data-testid="dashboard-action-receive"]'));
  if (await receiveBtn.isVisible()) {
    ...
    const formatButtons = page.locator(
      'button:has-text("UQ"), button:has-text("EQ"), button:has-text("0Q"), button:has-text("User-friendly"), button:has-text("Bounceable")',
    );
  ...
  for (const route of featureRoutes) {
    ...
    const tabs = page.locator(
      'button[data-testid^="personal-tab-"], button[data-testid^="dao-tab-"], button[data-testid^="brotherhood-tab-"], button[data-testid^="city-tab-"]',
    );
...
if (pageErrors.length > 0) {
  ...
  console.log(
    'SUCCESS: Zero runtime page errors detected during interactive audit!',
  );
  process.exit(0);
}
