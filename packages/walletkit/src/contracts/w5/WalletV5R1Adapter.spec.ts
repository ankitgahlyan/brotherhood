...
describe('WalletV5R1Adapter', () => {
  ...
  it('should create wallet with correct properties', async () => {
    ...
    expect(wallet.getAddress({ testnet: true })).toEqual(
      addressV5r1Test.bounceableNot,
    );
  ...
  it('should throw error if wallet contract not initialized', async () => {
    ...
    await expect(walletWithoutInit.getStateInit()).rejects.toThrow(
      'Wallet contract not properly initialized',
    );
  });
  ...
  it('should handle active/inactive/error states', async () => {
    ...
    expect(tonClient.getAccountState).toHaveBeenCalledWith(
      wallet.walletContract.address.toString(),
    );
    ...
    mocked(tonClient.getAccountState).mockRejectedValueOnce(
      new Error('Account state fetch failed'),
    );
    isDeployed = await wallet.isDeployed();
    expect(isDeployed).toEqual(false);
  });
