...
export class WalletV5 implements Contract {
  ...
  static createFromConfig(config: WalletV5Config, options: WalletOptions) {
    ...
    const wallet = new WalletV5(
      options.client,
      contractAddress(options.workchain, init),
      init,
    );
  ...
  get publicKey(): Promise<bigint> {
    return this.client
      .runGetMethod(asAddressFriendly(this.address), 'get_public_key')
      .then((data) => {
        if (data.exitCode === 0) {
  ...
  get status(): Promise<AccountStatus> {
    return this.client
      .getAccountState(asAddressFriendly(this.address))
  ...
  get isSignatureAuthAllowed(): Promise<boolean> {
    return this.client
      .runGetMethod(asAddressFriendly(this.address), 'is_signature_allowed')
      .then((data) => {
        if (data.exitCode === 0) {
          const parsedStack = ParseStack(data.stack);
          if (parsedStack[0]?.type === 'int') {
  ...
  get walletId(): Promise<WalletV5R1Id> {
    if (this.subwalletId !== undefined) {
      ...
      return this.client
        .runGetMethod(asAddressFriendly(this.address), 'get_subwallet_id')
        .then((data) => {
          if (data.exitCode === 0) {
            const parsedStack = ParseStack(data.stack);
            if (parsedStack[0]?.type === 'int') {
