...
export class RequestProcessor {
  ...
  async approveConnectRequest(
    ...
    if (event.embeddedRequest) {
      switch (event.embeddedRequest.method) {
        case 'sendTransaction':
          return this.approveConnectRequestSendTransactionEmbeddedRequest(
            event,
            response,
          );
        case 'signMessage':
          return this.approveConnectRequestSignMessageEmbeddedRequest(
            event,
            response,
          );
        case 'signData':
          return this.approveConnectRequestSignDataEmbeddedRequest(
            event,
            response,
          );
      }
    ...
    try {
      ...
      const tonConnectResponse = await this.createConnectApprovalResponse(
        event,
        response?.proof,
      );
  ...
  private async getTransactionRequestAndPreview(
    ...
    preview: TransactionEmulatedPreview | undefined;
  }> {
    if (!event.embeddedRequest) {
    ...
    if (
      event.embeddedRequest.method !== 'sendTransaction' &&
      event.embeddedRequest.method !== 'signMessage'
    ) {
      throw new WalletKitError(
        ERROR_CODES.INVALID_REQUEST_EVENT,
    ...
    const validation = validateTransactionRequestForWallet(
      transactionRequest,
      wallet,
      event.isLocal,
      false,
    ...
    const request = await checkTransactionRequestItems(
      transactionRequest,
      wallet,
    );
  ...
  private async approveConnectRequestSendTransactionEmbeddedRequest(
    ...
    const { request, preview } =
      await this.getTransactionRequestAndPreview(event);
    const connectionResult = await this.createConnectApprovalResponse(
      event,
      response?.proof,
    );
    ...
    return {
      ...
      request,
      connectionResult:
        connectionResult.result as unknown as EmbeddedConnectionResult,
    };
  }
  ...
  private validateWallet(event: ConnectionRequestEvent): Wallet {
    ...
    if (!wallet) {
      throw new WalletKitError(
        ...
        'Wallet not found for embedded request',
        undefined,
        {
          walletId,
          eventId: event.id,
   ...
   */
  private async createSessionForEmbeddedRequest(
    event: BridgeEvent,
  ...
  private async sendBridgeMessage(
    ...
    if (error) {
      const connResult =
        event.connectionResult as unknown as ConnectEventSuccess;
      const response = {
        ...connResult,
        response: { error: error.error } as ConnectEventSuccess['response'],
      };
    ...
    const response = {
      ...connResult,
      response: { result: result?.result } as ConnectEventSuccess['response'],
    };
    await this.bridgeManager.sendResponse(event, response);
    return;
  }
  ...
  async approveTransactionRequest(
    ...
    try {
      ...
      if ((error as { message: string })?.message?.includes('Ledger device')) {
        throw new WalletKitError(
          ERROR_CODES.LEDGER_DEVICE_ERROR,
          'Ledger device error',
          error as Error,
        );
      }
  ...
  async approveSignDataRequest(
    ...
    try {
      if (response) {
        ...
        const walletId = event.walletId;
        const walletAddress = getWalletAddressFromEvent(
          this.walletManager,
          event,
        );
  ...
  private async createConnectApprovalResponse(
    ...
    const deviceInfo = getDeviceInfoForWallet(
      wallet,
      this.walletKitOptions.deviceInfo,
    );
    ...
    const proofRequest = event.requestedItems.find(
      (item) => item.type === 'ton_proof',
    );
    if (proofRequest) {
...
async function createTonProofItem(
  params: CreateTonProofItemParams,
...
function toTonConnectSignDataPayload(
