...
export class StorageEventProcessor implements IEventProcessor {
  ...
  private async processNextAvailableEvent(): Promise<boolean> {
    try {
      ...
      const allSessions = allLocalSessions.filter(
        (session) =>
          session.walletId && this.registeredWallets.has(session.walletId),
      );
      ...
      if (allSessions.length > 0) {
        ...
        const walletIds = Array.from(
          new Set(allSessions.map((s) => s.walletId).filter(Boolean)),
        );
      ...
      if (noWalletEventTypes.length > 0) {
        const noWalletEvents =
          await this.eventStore.getNoWalletEvents(noWalletEventTypes);
      ...
      const walletId =
        allSessions.find((s) => s.sessionId === eventToUse.sessionId)
          ?.walletId || 'no-wallet';
  ...
  startRecoveryLoop(): void {
    ...
    const recoveryLoop = async () => {
      try {
        ...
        log.error('Error in recovery loop', {
          error: (error as Error).message,
        });
      }
  ...
  private async processEvent(
    ...
    if (!acquiredEvent) {
      log.debug('Failed to acquire lock on event', {
        eventId: event.id,
        walletId,
      });
      return false;
  ...
  private getNoWalletEnabledEventTypes(): EventType[] {
    ...
    return enabledTypes.filter(
      (type) => type === 'connect' || type === 'restoreConnection',
    );
  }
}
