...
export class Initializer {
  ...
  async initialize(
    ...
    try {
      ...
      const { requestProcessor } = this.initializeProcessors(
        sessionManager,
        bridgeManager,
        walletManager,
      );
  ...
  private async initializeManagers(
    ...
    if (options.sessionManager) {
      ...
      const storedSessionManager = new TONConnectStoredSessionManager(
        storage,
        walletManager,
      );
...
export async function wrapWalletInterface(
  ...
  const newProxy = new Proxy(wallet, {
    get: (target, prop) => {
      ...
      const ourMethonImplementation = ourClassesToExtend.find(
        (cls) => !!(cls.prototype as any)[prop],
      );
