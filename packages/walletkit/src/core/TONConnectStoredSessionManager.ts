...
export class TONConnectStoredSessionManager implements TONConnectSessionManager {
  ...
  async createSession(
    ...
    try {
      ...
      throw new Error(
        'Unable to resolve domain from dApp URL for new sessions',
      );
    }
  ...
  private async loadSessions(): Promise<void> {
    try {
      const storedSessions = await this.storage.get<TONConnectSession[]>(
        this.storageKey,
      );
        ...
        for (const session of storedSessions) {
          if (session.walletId && !session.walletAddress) {
            ...
            if (wallet) {
              ...
              log.warn('Session Wallet not found for session', {
  ...
  private async persistSessions(): Promise<void> {
    try {
      const sessionsToStore: TONConnectSession[] = Array.from(
