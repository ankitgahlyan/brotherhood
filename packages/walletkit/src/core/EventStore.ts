...
export class StorageEventStore implements EventStore {
  ...
  async storeEvent(_rawEvent: RawBridgeEvent): Promise<StoredEvent> {
    ...
    if (sizeBytes > MAX_EVENT_SIZE_BYTES) {
      throw new Error(
        `Event too large: ${sizeBytes} bytes (max: ${MAX_EVENT_SIZE_BYTES})`,
      );
    }
  ...
  async releaseLock(eventId: string, error?: string): Promise<StoredEvent> {
    return this.withLock('storage', async () => {
      ...
      if (event.status !== 'processing') {
        throw new Error(
          `Event not in processing status: ${eventId}, current status: ${event.status}`,
        );
      }
  ...
  private async getAllEventsFromStorage(): Promise<
    Record<string, StoredEvent>
  > {
    try {
      const eventsData = await this.storage.get<Record<string, StoredEvent>>(
        this.storageKey,
      );
      return eventsData || {};
    } catch (error) {
