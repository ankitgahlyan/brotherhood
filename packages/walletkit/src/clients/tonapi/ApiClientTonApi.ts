...
import type {
  TonApiDnsResolveResponse,
  TonApiDnsBackresolveResponse,
} from './types/dns';
...
import {
  mapTonApiGetMethodArgs,
  mapTonApiTvmStackRecord,
} from './mappers/map-methods';
...
import {
  mapTonApiTrace,
  mapTonApiTraceTransaction,
} from './mappers/map-traces';
...
export class ApiClientTonApi extends BaseApiClient implements ApiClient {
  ...
  async jettonsByAddress(
    ...
    const raw = await this.getJson<TonApiJettonInfo>(
      `/v2/jettons/${request.address}`,
    );
  ...
  async runGetMethod(
    ...
    if (!raw.success) {
      throw new Error(
        `TonApi runGetMethod '${method}' failed with exit code ${raw.exit_code}`,
      );
    }
  ...
  async getTransactionsByHash(
    ...
    const byTransaction = async () =>
      this.getJson<TonApiTransaction>(
        `/v2/blockchain/transactions/${normalizedHash}`,
      );
  ...
  async getTrace(request: GetTraceRequest): Promise<ToncenterTracesResponse> {
    const candidates =
      request.traceId && request.traceId.length > 0 ? request.traceId : [];
    if (request.account) {
  ...
  async resolveDnsWallet(domain: string): Promise<string | undefined> {
    try {
      const raw = await this.getJson<TonApiDnsResolveResponse>(
        `/v2/dns/${domain}/resolve`,
      );
  ...
  private normalizeTonApiId(value: string): string {
    ...
    if (
      /^[0-9a-fA-F]+$/.test(normalizedValue) &&
