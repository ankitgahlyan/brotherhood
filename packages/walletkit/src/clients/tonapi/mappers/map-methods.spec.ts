...
describe('map-methods', () => {
  describe('mapTonApiGetMethodArgs', () => {
    ...
    it('should map hex num to int257', () => {
      ...
      expect(mapTonApiGetMethodArgs(stack)).toEqual([
        { type: 'int257', value: '0x1A' },
      ]);

      const negativeStack: RawStackItem[] = [{ type: 'num', value: '-0x1A' }];
      expect(mapTonApiGetMethodArgs(negativeStack)).toEqual([
        { type: 'int257', value: '-0x1A' },
      ]);
    });
    ...
    it('should map cell', () => {
      ...
      expect(mapTonApiGetMethodArgs(stack)).toEqual([
        { type: 'cell_boc_base64', value: 'base64boc' },
      ]);
    });
    ...
    it('should map slice to slice_boc_hex', () => {
      ...
      expect(mapTonApiGetMethodArgs(stack)).toEqual([
        { type: 'slice_boc_hex', value: hexStr },
      ]);
    });
    ...
    it('should reject tuple and list', () => {
      expect(() =>
        mapTonApiGetMethodArgs([{ type: 'tuple', value: [] }]),
      ).toThrow("TonApi doesn't support tuple in get method arguments");
      expect(() =>
        mapTonApiGetMethodArgs([{ type: 'list', value: [] }]),
      ).toThrow("TonApi doesn't support list in get method arguments");
    });

    it('should reject unknown type', () => {
      // @ts-expect-error testing unknown type
      expect(() => mapTonApiGetMethodArgs([{ type: 'unknown' }])).toThrow(
  ...
  describe('mapTonApiTvmStackRecord', () => {
    ...
    it('should map slice with fallback cell field', () => {
      const hexStr = Buffer.from('test slice fallback', 'utf-8').toString(
        'hex',
      );
      const base64Str = Buffer.from('test slice fallback', 'utf-8').toString(
        'base64',
      );
    ...
    it('should reject unknown type', () => {
      ...
      // @ts-expect-error testing unknown type
      expect(() => mapTonApiTvmStackRecord(item)).toThrow(
        'Unsupported TonApi stack item type: unknown',
      );
    });
  });
});
