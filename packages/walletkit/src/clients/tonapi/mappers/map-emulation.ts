...
function mapTraceNode(trace: TonApiTrace): EmulationTraceNode {
  return {
    ...
    inMsgHash: trace.transaction.in_msg?.hash
      ? toHex(trace.transaction.in_msg.hash)
      : undefined,
...
function mapMessage(raw: TonApiMessage, kind: 'in' | 'out'): EmulationMessage {
  ...
  return {
    ...
    source: raw.source?.address
      ? asAddressFriendly(raw.source.address)
      : undefined,
    destination: raw.destination?.address
      ? (asAddressFriendly(raw.destination.address) ?? '')
      : '',
    value: isExternal
      ? undefined
      : raw.value != null
    ...
    createdLt: isExternal
      ? undefined
      : raw.created_lt != null
        ? String(raw.created_lt)
        : undefined,
    createdAt: isExternal
      ? undefined
      : raw.created_at != null
...
function mapTransaction(
  ...
  const outMsgs =
    raw.out_msgs && raw.out_msgs.length > 0 ? raw.out_msgs : childInMsgs;
  ...
  return {
    ...
    prevTransLt:
      raw.prev_trans_lt != null ? String(raw.prev_trans_lt) : undefined,
    ...
    description: {
      ...
      storagePhase: {
        storageFeesCollected: String(
          raw.storage_phase?.storage_fees_collected ?? 0,
        ),
        statusChange: normalizeStatusChange(raw.storage_phase?.status_change),
      },
      creditPhase:
        raw.credit_phase?.credit != null
...
function normalizeJettonTransferDetails(
  ...
  return {
    ...
    sender_jetton_wallet:
      (payload.senders_wallet as string | undefined) ?? null,
    receiver_jetton_wallet:
      (payload.recipients_wallet as string | undefined) ?? null,
    amount: payload.amount ?? '0',
    comment: (payload.comment as string | undefined) ?? null,
    is_encrypted_comment:
      (payload.is_encrypted_comment as boolean | undefined) ?? false,
...
function _mapAction(
  ...
  const actionId = toHex(
    String(action.base_transactions?.[0] ?? event.event_id),
  );
  const transactions = (action.base_transactions ?? []).map((h) =>
    toHex(String(h)),
  );
  ...
  if (type === 'TonTransfer' && payload) {
    ...
    details = {
      source:
        (payload.sender as { address?: string } | undefined)?.address ?? '',
      destination:
        (payload.recipient as { address?: string } | undefined)?.address ?? '',
    ...
    type = 'jetton_swap';
    const inAsset = payload.in as
      { jetton?: { address?: string }; amount?: string } | undefined;
    const outAsset = payload.out as
      { jetton?: { address?: string }; amount?: string } | undefined;
    ...
    details = {
      ...
      dex_incoming_transfer: {
        ...
        amount: String(
          payload.amount_in ?? payload.ton_in ?? inAsset?.amount ?? 0,
        ),
      },
      dex_outgoing_transfer: {
        ...
        destination_jetton_wallet: null,
        amount: String(
          payload.amount_out ?? payload.ton_out ?? outAsset?.amount ?? 0,
        ),
      },
      peer_swaps: [],
    };
...
export function mapTonApiEmulationResponse(
  result: TonApiMessageConsequences,
): EmulationResponse {
  ...
  const externalHash = result.transaction.in_msg?.hash
    ? toHex(result.transaction.in_msg.hash)
    : rootTxHash;
      ...
      if (traceNode.children) {
        for (const child of traceNode.children) {
          ...
          if (childTransaction.inMsg) {
            ...
            if (
              !mappedTransaction.outMsgs.some(
                (m) => m.hash === childTransaction?.inMsg?.hash,
              )
            ) {
