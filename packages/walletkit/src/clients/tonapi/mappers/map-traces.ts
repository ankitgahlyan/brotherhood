...
export function mapTraceStatus(
  status: string | undefined,
): 'active' | 'frozen' | 'uninit' | string {
...
export function mapTonApiTraceMessage(raw: TonApiMessage) {
  ...
  return {
    ...
    value:
      raw.value !== undefined && raw.value !== null ? String(raw.value) : null,
    value_extra_currencies: extraCurrencies,
    fwd_fee:
      raw.fwd_fee !== undefined && raw.fwd_fee !== null
        ? String(raw.fwd_fee)
        : null,
    ihr_fee:
      raw.ihr_fee !== undefined && raw.ihr_fee !== null
        ? String(raw.ihr_fee)
        : null,
    created_lt:
      raw.created_lt !== undefined && raw.created_lt !== null
    ...
    import_fee:
      raw.import_fee !== undefined && raw.import_fee !== null
...
export function mapTonApiTraceTransaction(
  ...
  return {
    ...
    prev_trans_lt:
      raw.prev_trans_lt !== undefined && raw.prev_trans_lt !== null
        ? String(raw.prev_trans_lt)
        : null,
    ...
    description: {
      ...
      credit_ph:
        raw.credit_phase?.credit !== undefined &&
...
export function mapTonApiTrace(
  ...
  const transactionsOrder = [...traceNodes]
    .sort((a, b) =>
      BigInt(a.transaction.lt ?? 0) < BigInt(b.transaction.lt ?? 0) ? -1 : 1,
    )
  ...
  const startLt =
    lts.length > 0
      ? lts.reduce((min, value) => (value < min ? value : min), lts[0])
      : 0n;
  const endLt =
    lts.length > 0
      ? lts.reduce((max, value) => (value > max ? value : max), lts[0])
      : 0n;
  ...
  const messagesCount = traceNodes.reduce(
    (acc, node) =>
      acc +
      (node.transaction.in_msg ? 1 : 0) +
      (node.transaction.out_msgs?.length ?? node.children?.length ?? 0),
    0,
  );
