...
import type {
  TonApiTvmStackRecord,
  TonApiExecGetMethodArg,
} from '../types/methods';
 ...
 */
export const mapTonApiGetMethodArgs = (
  stack?: RawStackItem[],
): TonApiExecGetMethodArg[] => {
  return (stack || []).map((item) => {
    switch (item.type) {
      ...
      case 'slice':
        ...
        return {
          type: 'slice_boc_hex',
          value: Buffer.from(item.value, 'base64').toString('hex'),
        };
      case 'builder':
        return { type: 'cell_boc_base64', value: item.value };
      case 'tuple':
      case 'list':
        throw new Error(
          `TonApi doesn't support ${item.type} in get method arguments`,
        );
      default:
        throw new Error(
          `Unsupported stack item type: ${(item as { type: string }).type}`,
        );
    }
  });
};

export const mapTonApiTvmStackRecord = (
  item: TonApiTvmStackRecord,
): RawStackItem => {
  switch (item.type) {
    case 'null':
      return { type: 'null' };
    ...
    case 'tuple':
      return {
        type: 'tuple',
        value: (item.tuple || []).map(mapTonApiTvmStackRecord),
      };
    default:
