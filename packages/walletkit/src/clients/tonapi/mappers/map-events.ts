...
import type {
  TonApiAccountEvent,
  TonApiSimplePreviewAccount,
} from '../types/events';
import { toHex } from './map-transactions';

export function normalizeTonApiAccountAddress(
  account: TonApiSimplePreviewAccount,
): string {
  ...
  return {
    ...
    actions: (raw.actions ?? []).map((action) => {
      const status: 'success' | 'failure' =
        action.status === 'failed' ? 'failure' : 'success';
      ...
      return {
        ...
        baseTransactions: (action.base_transactions ?? []).map(
          (transactionHash) => toHex(String(transactionHash)),
        ),
