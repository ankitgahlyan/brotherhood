...
import {
  ...
  makeNonExistingAccountState,
} from './mappers/map-account-states-entry';
...
export class ApiClientToncenter extends BaseApiClient implements ApiClient {
  ...
  async nftItemsByAddress(request: NFTsRequest): Promise<NFTsResponse> {
    ...
    const response = await this.getJson<NftItemsResponseV3>(
      '/api/v3/nft/items',
      props,
    );
  ...
  async fetchEmulation(
    ...
    const response = await this.postJson<ToncenterEmulationResponse>(
  ...
  async sendBoc(boc: Base64String): Promise<string> {
    ...
    const response = await this.postJson<V2SendMessageResult>(
      '/api/v3/message',
      { boc },
    );
  ...
  async runGetMethod(
    ...
    const raw = await this.postJson<V3RunGetMethodRequest>(
      '/api/v3/runGetMethod',
      props,
    );
    return {
  ...
  async getAccountStates(
    ...
    for (const inputAddr of uniqueAddrs) {
      const account = raw.accounts.find((a) =>
        compareAddress(a.address, inputAddr),
      );
  ...
  async getTransactionsByHash(
    ...
    const msgHash =
      'msgHash' in request ? padBase64(request.msgHash) : undefined;
    const bodyHash =
      'bodyHash' in request ? padBase64(request.bodyHash) : undefined;
  ...
  async getPendingTransactions(
    ...
    const accounts =
      'accounts' in request ? request.accounts?.map(prepareAddress) : undefined;
  ...
  async getTrace(request: GetTraceRequest): Promise<ToncenterTracesResponse> {
    ...
    const traceId = isHexId
      ? traceIdStr
      : padBase64(Base64Normalize(traceIdStr).replace(/=/g, ''));
  ...
  async getPendingTrace(
    request: GetPendingTraceRequest,
  ): Promise<ToncenterTracesResponse> {
    try {
      const response = await CallForSuccess(
        () => {
