...
export function toTransactionsResponse(
  response: ToncenterTransactionsResponse,
): TransactionsResponse {
  return {
...
function toTransaction(tx: ToncenterTransaction): Transaction {
  return {
    ...
    previousTransactionHash: tx.prev_trans_hash
...
function toTransactionDescription(
  desc: EmulationTransactionDescription,
): TransactionDescription {
  return {
    type: desc.type,
  ...
  return {
    ...
    messageContent: {
      ...
      body: msg.message_content?.body
        ? (msg.message_content.body as Base64String)
        : undefined,
...
function toAccountState(state: EmulationAccountState): TransactionAccountState {
  return {
    ...
    accountStatus: state.account_status
      ? toAccountStatus(state.account_status)
      : undefined,
