...
export function toTransactionEmulatedTrace(
  ...
  return {
    ...
    transactions: Object.fromEntries(
      Object.entries(response.transactions ?? {}).map(([hash, tx]) => [
        hash,
        emulationTxToTransaction(tx),
      ]),
    ),
    ...
    codeCells: Object.fromEntries(
      Object.entries(response.codeCells ?? {}).map(([hash, cell]) => [
        hash,
        cell,
      ]),
    ),
    dataCells: Object.fromEntries(
      Object.entries(response.dataCells ?? {}).map(([hash, cell]) => [
        hash,
        cell,
      ]),
    ),
...
function domainTraceNodeToTransactionTraceNode(
  node: DomainEmulationTraceNode,
): TransactionTraceNode {
  return {
    txHash: node.txHash,
...
function emulationAccountStateToAccountState(
  state: DomainEmulationAccountState,
): TransactionAccountState {
  return {
...
function emulationMsgToTransactionMessage(
  msg: DomainEmulationMessage,
): TransactionMessage {
  return {
    hash: msg.hash,
...
function emulationDescToTransactionDescription(
  desc: DomainEmulationTransactionDescription,
): TransactionDescription {
  return {
    type: desc.type,
    ...
    creditPhase: desc.creditPhase
      ? { credit: desc.creditPhase.credit }
      : undefined,
    computePhase: {
...
function emulationTxToTransaction(tx: DomainEmulationTransaction): Transaction {
  return {
    ...
    inMessage: tx.inMsg
      ? emulationMsgToTransactionMessage(tx.inMsg)
      : undefined,
    outMessages: tx.outMsgs.map(emulationMsgToTransactionMessage),
    accountStateBefore: emulationAccountStateToAccountState(
      tx.accountStateBefore,
    ),
    accountStateAfter: emulationAccountStateToAccountState(
      tx.accountStateAfter,
    ),
...
function emulationActionToTransactionTraceAction(
  action: DomainEmulationAction,
): TransactionTraceAction {
  return {
    ...
    details: domainActionDetailsToTransactionTraceActionDetails(
      action.type,
      action.details,
    ),
  };
...
function domainActionDetailsToTransactionTraceActionDetails(
  ...
  if (type === 'jetton_swap') {
    return {
      type: 'jetton_swap',
      value: toTransactionTraceActionJettonSwapDetails(
        details as unknown as EmulationJettonSwapDetails,
      ),
    };
  } else if (type === 'call_contract') {
    return {
      type: 'call_contract',
      value: toTransactionTraceActionCallContractDetails(
        details as unknown as EmulationCallContractDetails,
      ),
    };
  } else if (type === 'ton_transfer') {
    return {
      type: 'ton_transfer',
      value: toTransactionTraceActionTONTransferDetails(
        details as unknown as EmulationTonTransferDetails,
      ),
    };
  } else {
...
function emulationAddressBookToAddressBook(
  book: Record<string, DomainEmulationAddressBookEntry>,
): AddressBook {
...
function toTransactionTraceActionJettonSwapDetails(
  ...
  return {
    ...
    dexIncomingTransfer: {
      asset:
        asMaybeAddressFriendly(details.dex_incoming_transfer?.asset) ??
        undefined,
      source:
        asMaybeAddressFriendly(details.dex_incoming_transfer?.source) ??
        undefined,
      destination:
        asMaybeAddressFriendly(details.dex_incoming_transfer?.destination) ??
        undefined,
      sourceJettonWallet:
        asMaybeAddressFriendly(
          details.dex_incoming_transfer?.source_jetton_wallet,
        ) ?? undefined,
      destinationJettonWallet:
        asMaybeAddressFriendly(
          details.dex_incoming_transfer?.destination_jetton_wallet,
        ) ?? undefined,
      amount: details.dex_incoming_transfer?.amount,
    },
    dexOutgoingTransfer: {
      asset:
        asMaybeAddressFriendly(details.dex_outgoing_transfer?.asset) ??
        undefined,
      source:
        asMaybeAddressFriendly(details.dex_outgoing_transfer?.source) ??
        undefined,
      destination:
        asMaybeAddressFriendly(details.dex_outgoing_transfer?.destination) ??
        undefined,
      sourceJettonWallet:
        asMaybeAddressFriendly(
          details.dex_outgoing_transfer?.source_jetton_wallet,
        ) ?? undefined,
      destinationJettonWallet:
        asMaybeAddressFriendly(
          details.dex_outgoing_transfer?.destination_jetton_wallet,
        ) ?? undefined,
