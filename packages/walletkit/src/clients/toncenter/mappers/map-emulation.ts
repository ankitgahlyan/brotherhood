    ...
    actionPhase: desc.action
      ? {
          ...
          actionListHash: desc.action.action_list_hash
            ? Base64ToHex(desc.action.action_list_hash)
            : undefined,
          totalMsgSize: {
  ...
  return {
    ...
    messageContent: {
      ...
      body: msg.message_content?.body
        ? asBase64(msg.message_content.body)
        : undefined,
...
function mapTransaction(tx: RawTransaction): EmulationTransaction {
  return {
    ...
    prevTransHash: tx.prev_trans_hash
      ? Base64ToHex(tx.prev_trans_hash)
      : undefined,
...
export function mapToncenterEmulationResponse(
  raw: ToncenterEmulationResponse,
): EmulationResponse {
  const transactions: Record<string, EmulationTransaction> = Object.fromEntries(
    Object.entries(raw.transactions ?? {}).map(([hash, tx]) => [
      Base64ToHex(hash),
      mapTransaction(tx),
    ]),
  );
  ...
  const codeCells: Record<string, Base64String> = Object.fromEntries(
    Object.entries(raw.code_cells ?? {}).map(([hash, cell]) => [
      Base64ToHex(hash),
      asBase64(cell),
    ]),
  );
  ...
  const dataCells: Record<string, Base64String> = Object.fromEntries(
    Object.entries(raw.data_cells ?? {}).map(([hash, cell]) => [
      Base64ToHex(hash),
      asBase64(cell),
    ]),
  );
