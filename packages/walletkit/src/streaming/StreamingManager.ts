...
> implements StreamingAPI {
  ...
  registerProvider(input: ProviderInput<StreamingProvider>): void {
    ...
    if (this.providers.has(networkId)) {
      log.warn(
        `Provider for network ${networkId} is already registered. Overriding.`,
      );
  ...
  watchBalance(
    ...
    return this.getProvider(network).watchBalance(
  ...
  watchTransactions(
    ...
    return this.getProvider(network).watchTransactions(
      asAddressFriendly(address),
      onChange,
    );
  }
  ...
  watchJettons(
    ...
    return this.getProvider(network).watchJettons(
      asAddressFriendly(address),
      onChange,
    );
  }
  ...
  watch(
    ...
    const unwatchers = types.map((type) => {
      switch (type) {
        case 'balance':
          return this.watchBalance(network, address, (u) =>
            onUpdate('balance', u),
          );
        case 'transactions':
          return this.watchTransactions(network, address, (u) =>
            onUpdate('transactions', u),
          );
        case 'jettons':
          return this.watchJettons(network, address, (u) =>
            onUpdate('jettons', u),
          );
        default:
          return () => {};
  ...
  private getProvider(network: Network): StreamingProvider {
    ...
    if (!provider) {
      throw new Error(
        `No streaming provider registered for network ${networkId}`,
      );
    }
    return provider;
  }
