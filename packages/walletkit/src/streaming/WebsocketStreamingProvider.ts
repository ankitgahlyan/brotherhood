...
export abstract class WebsocketStreamingProvider implements StreamingProvider {
  ...
  private balanceCallbacks: Map<string, Set<(update: BalanceUpdate) => void>> =
    new Map();
  private transactionCallbacks: Map<
    string,
    Set<(update: TransactionsUpdate) => void>
  > = new Map();
  private jettonCallbacks: Map<string, Set<(update: JettonUpdate) => void>> =
    new Map();

  private connectionChangeCallbacks: Set<(connected: boolean) => void> =
    new Set();
  ...
  protected hasActiveSubscriptions(): boolean {
    return (
      this.balanceCallbacks.size > 0 ||
      this.transactionCallbacks.size > 0 ||
      this.jettonCallbacks.size > 0
    );
  }
  ...
  protected scheduleReconnect(): void {
    ...
    log.info(
      `Scheduling reconnect in ${delayMs}ms (attempt ${this.reconnectAttempts})`,
    );
