...
import type {
  StreamingV2AccountState,
  StreamingV2TransactionRaw,
  StreamingV2TransactionDescription,
} from '../types';
import type {
  EmulationBlockRef,
  EmulationMessage,
...
const toTransactionMessage = (msg: EmulationMessage): TransactionMessage => {
  return {
    ...
    messageContent: msg.message_content
      ? {
          hash: msg.message_content.hash
            ? Base64ToHex(msg.message_content.hash)
            : undefined,
...
const toComputePhase = (
  desc: StreamingV2TransactionDescription,
): TransactionComputePhase => {
  ...
  return {
    ...
    vmInitStateHash: full.vm_init_state_hash
      ? Base64ToHex(full.vm_init_state_hash)
      : undefined,
    vmFinalStateHash: full.vm_final_state_hash
      ? Base64ToHex(full.vm_final_state_hash)
      : undefined,
  };
};

const toTransactionDescription = (
  desc: StreamingV2TransactionDescription,
): TransactionDescription => {
  ...
  return {
    ...
    action: action
      ? {
          ...
          actionListHash: action.action_list_hash
            ? Base64ToHex(action.action_list_hash)
            : undefined,
 ...
 */
export const toStreamingTransaction = (
  ...
  return {
    ...
    previousTransactionHash: raw.prev_trans_hash
      ? Base64ToHex(raw.prev_trans_hash)
      : undefined,
