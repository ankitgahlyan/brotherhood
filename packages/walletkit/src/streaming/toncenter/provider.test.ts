...
const ADDR_A =
  '0:83dfd552e63729b472fcbcc8c44e6cc6691702558b68ecb527e1ba403a0f31a8';
const ADDR_B =
  '0:ef4458951c1468a43d5506def6543b009c1fd48392497b45453287efdfa40f05';

const makeContext = (): ProviderFactoryContext => ({
  networkManager: {} as any,
  eventEmitter: {} as any,
});
...
describe('TonCenterStreamingProvider', () => {
  ...
  it('appends api_key to URL when provided', () => {
    ...
    expect(MockWebSocket.lastInstance?.url).toContain(
      'toncenter.com/api/streaming/v2/ws',
    );
  ...
  it('debounces multiple watch calls and sends monolithic subscriptions', async () => {
    ...
    const sentMessages = MockWebSocket.lastInstance?.send.mock.calls.map(
      (call) => JSON.parse(call[0]),
    );
    const subscribeMsgs = sentMessages?.filter(
      (m) => m.operation === 'subscribe',
    );
  ...
  it('sends new subscription without removed address when one watcher is removed', async () => {
    ...
    const lastSubscribe = sentMessages
  ...
  it('handles rapid watch/unwatch for the same address gracefully via debouncing', async () => {
    ...
    const lastMsg = JSON.parse(
      MockWebSocket.lastInstance?.send.mock.calls.pop()![0],
    );
  ...
  describe('Finality and Invalidation', () => {
    ...
    beforeEach(() => {
      txCbA = vi.fn();
      txCbB = vi.fn();
      provider = new TonCenterStreamingProvider(makeContext(), {
        network: Network.testnet(),
      });
    ...
    it('ignores notifications with lower finality than cached', () => {
      ...
      provider.onMessage({
        data: JSON.stringify(confirmedMsg),
      } as MessageEvent);
    ...
    it('notifies all participating accounts on trace_invalidated', () => {
      ...
      // @ts-expect-error accessing protected
      provider.onMessage({
        data: JSON.stringify(invalidateMsg),
      } as MessageEvent);
      expect(txCbA).toHaveBeenCalledTimes(2);
