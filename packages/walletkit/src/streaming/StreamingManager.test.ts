...
interface MockProvider extends StreamingProvider {
  watchBalance: Mock<
    (address: string, onChange: (update: BalanceUpdate) => void) => () => void
  >;
...
const makeMockProvider = (
  network: Network = makeMockNetwork(),
): MockProvider => {
...
const makeFactoryCtx = () =>
  vi.fn(() => ({ networkManager: {} as any, eventEmitter: {} as any }))();
...
const makeManager = (provider: MockProvider) => {
  ...
  return {
    manager,
    factory: factory as unknown as Mock<StreamingProviderFactory>,
  };
};
