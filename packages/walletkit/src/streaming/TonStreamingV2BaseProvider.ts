...
import {
  isTransactionsNotification,
  isTraceInvalidatedNotification,
} from './toncenter/guards/transaction';
...
export abstract class TonStreamingV2BaseProvider extends WebsocketStreamingProvider {
  ...
  private traceCache = new LRUCache<
    string,
    { score: number; accounts: Set<string> }
  >({ max: 10000 });

  constructor(
    _ctx: ProviderFactoryContext,
    options: TonStreamingV2BaseOptions,
  ) {
    super();
    ...
    try {
      ...
      if (isTransactionsNotification(msg)) {
        ...
        if (entry && finalityScore < entry.score) {
          log.debug(
            'Ignoring transactions notification due to lower finality',
            {
