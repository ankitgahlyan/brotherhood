...
export class WindowAccessor {
  ...
  constructor(
    window: Window,
    { bridgeKey, injectTonKey }: { bridgeKey: string; injectTonKey?: boolean },
  ) {
    this.window = window;
  ...
  exists(): boolean {
    ...
    return !!(
      ...
      (windowObj[this.bridgeKey] as Record<string, unknown>).tonconnect
    );
  }
  ...
  injectBridge(bridge: TonConnectBridge): void {
    ...
    const windowObj = this.window as unknown as Record<
      string,
