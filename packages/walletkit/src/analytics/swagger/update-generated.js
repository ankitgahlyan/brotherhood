      ...
      for (const [key, value] of Object.entries(obj)) {
        if (key === 'enum' && Array.isArray(value) && obj.type === 'string') {
          ...
          if (nonEmptyValues.length > 0) {
            ...
            delete result.enum;
            result.anyOf = [
              ...nonEmptyValues.map((val) => ({ const: val })),
              { type: 'string' },
            ];
          } else {
            result[key] = value;
...
function transformEventName(content) {
  ...
  const pattern =
    /"event_name":\s*\{\s*"type":\s*"string",\s*"enum":\s*\[[^\]]+\],\s*"example":\s*"([^"]+)"\s*\}/g;
      ...
      if (
        ...
        for (const field of requiredFields) {
          if (
            ...
            !definition.required.includes(field)
          ) {
        ...
        const eventName =
          definition.properties.event_name?.const ||
          definition.properties.event_name?.example;
...
async function main() {
  ...
  try {
    ...
    const newPathKeys = Object.keys(parsedContent.paths).filter(
      (key) => key === '/events',
    );
    const newPaths = {};
    ...
    const originalMatches = content.match(
      /"event_name":\s*\{\s*"type":\s*"string",\s*"example":\s*"[^"]+"\s*\}/g,
    );
