...
import {
  ...
  getWalletFromEvent,
  parseTonConnectTransactionRequest,
} from '../utils/events';
...
export class TransactionHandler
  ...
  implements
    EventHandler<SendTransactionRequestEvent, RawBridgeEventTransaction>
{
  ...
  async handle(
  ...
  ): Promise<SendTransactionRequestEvent | WalletResponseTemplateError> {
    ...
    const requestValidation = this.parseTonConnectTransactionRequest(
      event,
      wallet,
    );
    if (!requestValidation.result || !requestValidation?.validation?.isValid) {
      log.error('Failed to parse transaction request', {
        event,
        requestValidation,
      });
