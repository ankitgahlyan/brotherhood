  ...
  async handle(event: RawBridgeEventSignData): Promise<SignDataRequestEvent> {
    ...
    if (!walletId && !walletAddress) {
      throw new WalletKitError(
        ...
        'No wallet ID found in sign data event',
        undefined,
        {
          eventId: event.id,
        },
    ...
    const wallet = walletId
      ? this.walletManager.getWallet(walletId)
      : undefined;
    ...
    if (!payload) {
      ...
      throw new WalletKitError(
        ...
        'No data to sign found in request',
        undefined,
        {
          eventId: event.id,
        },
