  ...
  implements EventHandler<ConnectionRequestEvent, RawBridgeEventConnect>
{
  ...
  async handle(event: RawBridgeEventConnect): Promise<ConnectionRequestEvent> {
    ...
    const connectEvent: ConnectionRequestEvent = {
      ...
      requestedItems: event.params.items
        ? this.toConnectionRequestEventRequestedItems(event.params.items)
        : [],
      preview,
  ...
  private toConnectionRequestEventRequestedItems(
    items: ConnectItem[],
  ): ConnectionRequestEventRequestedItem[] {
  ...
  private extractDAppName(event: RawBridgeEvent, manifest?: any): string {
    const name =
      manifest?.name || (event as any).params?.manifest?.name || 'Unknown dApp';
  ...
  private createPreview(
      ...
      if (!finalManifestFetchErrorCode && dAppUrl) {
        try {
          ...
          if (!isValidHost(parsedDAppUrl.host)) {
            ...
            finalManifestFetchErrorCode =
              CONNECT_EVENT_ERROR_CODES.MANIFEST_CONTENT_ERROR;
          }
        } catch (_) {
          log.warn('Invalid dApp URL in manifest - failed to parse', {
            dAppUrl,
          });
          finalManifestFetchErrorCode =
            CONNECT_EVENT_ERROR_CODES.MANIFEST_CONTENT_ERROR;
    ...
    const permissions = [];
    const addrItem = requestedItems.find(
      (item: ConnectItem) => item.name === 'ton_addr',
    );
    if (addrItem) {
      permissions.push({
        name: 'ton_addr',
    ...
    const proofItem = requestedItems.find(
      (item: ConnectItem) => item.name === 'ton_proof',
    );
    if (proofItem) {
      permissions.push({
        name: 'ton_proof',
        title: 'TON Proof',
        description:
          'Gives dApp signature, that can be used to verify your access to private key',
      });
    }
  ...
  private async fetchManifest(
    ...
    if (
      this.config.fetchManifest &&
      typeof this.config.fetchManifest === 'function'
    ) {
