...
export function getDeviceInfoWithDefaults(
  options?: Partial<DeviceInfo>,
): DeviceInfo {
...
export function createDeviceInfo(options?: Partial<DeviceInfo>): DeviceInfo {
  ...
  if (
    infoWithDefaults.features.some(
      (feature) =>
    ...
    infoWithDefaults.features.some((feature) => feature === 'SendTransaction')
  ) {
    features.push('SendTransaction');
  }
...
export function getWalletInfoWithDefaults(
  options?: Partial<WalletInfo>,
): WalletInfo {
...
function addLegacySendTransactionFeature(options: DeviceInfo): DeviceInfo {
  ...
  const hasSendTransactionString = features.some(
    (feature) => feature === 'SendTransaction',
  );
