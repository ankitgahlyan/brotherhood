...
describe('Cells parsing', () => {
  it('toStringTail', async () => {
    ...
    expect(toStringTail('s'.repeat(2)).toString()).toEqual(
      `x{${'73'.repeat(2)}}`,
    );
    expect(toStringTail('s'.repeat(555)).toString()).toEqual(
      `x{${'73'.repeat(127)}}`,
    );
  });
  ...
  it('fromTinyString', async () => {
    expect(
      fromTinyString(
        Cell.fromHex('b5ee9c7241010101000600000803746f6e39288b0f'),
      ).toString(),
    ).toEqual('ton');
  });
});
