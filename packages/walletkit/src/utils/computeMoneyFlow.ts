...
function parseJettonTransfer(slice: Slice): RawJettonTransfer {
  if (
    ...
    slice.preloadUint(32) !== JETTON_TRANSFER_OPCODE
  ) {
...
export async function computeMoneyFlow(
  ...
  const ourTxs = Object.values(response.transactions).filter(
    (tx) => tx.account === ourAddress,
  );

  const outputs = ourTxs.reduce(
    (acc, tx) => tx.outMsgs.reduce((a, m) => a + BigInt(m.value ?? 0), acc),
    0n,
  );

  const incomingTxes = options.skipFirstTxInput
    ? ourTxs.filter((t) => t.hash !== rootTx.hash)
    : ourTxs;
  const inputs = incomingTxes.reduce(
    (acc, tx) => (tx.inMsg?.value ? acc + BigInt(tx.inMsg.value) : acc),
    0n,
  );
  ...
  for (const t of Object.values(response.transactions)) {
    ...
    const to =
      parsed.destination instanceof Address ? parsed.destination : null;
    if (!to) {
      continue;
    }
    ...
    try {
      jettonMasterAddress = await getJettonMasterAddressFromClient(
        client,
        t.account,
      );
    } catch (_) {
  ...
  const ourJettonTransfersByAddress = jettonTransfers.reduce<
  ...
  const ourJettonTransfers: TransactionTraceMoneyFlowItem[] = Object.entries(
    ourJettonTransfersByAddress,
  ).map(([jettonKey, amount]) => ({
    assetType: AssetType.jetton,
  ...
  return {
    ...
    ourTransfers: [
      { assetType: AssetType.ton, amount: (inputs - outputs).toString() },
      ...ourJettonTransfers,
    ],
    ourAddress,
  };
