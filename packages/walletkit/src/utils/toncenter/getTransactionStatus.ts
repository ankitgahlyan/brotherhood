...
export async function getTransactionStatus(
  client: ApiClient,
  params:
    | { boc: string; normalizedHash?: never }
    | { boc?: never; normalizedHash: string },
): Promise<TransactionStatusResponse> {
  const hashToSearch = params.boc
    ? getNormalizedExtMessageHash(params.boc).hash
  ...
  try {
    ...
    const pendingResponse = await client.getPendingTrace({
      externalMessageHash: [hashToSearch],
    });
