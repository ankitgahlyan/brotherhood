...
const TON_MNEMONIC_PUBLIC_KEY = Uint8Array.from(
  Buffer.from(
    '2fea08c108702b69531fc793953466e690f2076c64c0393948aae4177dd2a9f5',
    'hex',
  ),
);
const TON_MNEMONIC_SECRET_KEY = Uint8Array.from(
  Buffer.from(
    '0e5d1af976fc42954764144d47f6a4d38e1b753a22fda8173be659edddfd00ed',
    'hex',
  ),
);
...
const BIP39_MNEMONIC_PUBLIC_KEY = Uint8Array.from(
  Buffer.from(
    'd2ce1d2618ee59ecdcc2b55b9f05b2eea8729e98a49306b1a04c6510074ecc9f',
    'hex',
  ),
);
const BIP39_MNEMONIC_SECRET_KEY = Uint8Array.from(
  Buffer.from(
    '098cb1f3427de537ea4a4ef682e54d8cdd69d0f51ddb37f7f2ae48ffa1445ea1',
    'hex',
  ),
);
  ...
  describe('MnemonicToKeyPair', () => {
    describe('with TON mnemonic type', () => {
      ...
      it('should use TON type by default', async () => {
        const resultWithType = await MnemonicToKeyPair(
          VALID_TON_MNEMONIC_24,
          'ton',
        );
        ...
        expect(resultWithType.secretKey.slice(0, 32)).toEqual(
          TON_MNEMONIC_SECRET_KEY,
        );
      });
      ...
      it('should produce different keys for different mnemonics', async () => {
        ...
        expect(
          Buffer.from(result1.publicKey).equals(Buffer.from(result2.publicKey)),
        ).toBe(false);
      });
    });
    ...
    describe('with BIP39 mnemonic type', () => {
      it('should convert 24-word mnemonic array to key pair', async () => {
        ...
        expect(result.secretKey.slice(0, 32)).toEqual(
          BIP39_MNEMONIC_SECRET_KEY,
        );
      });
      ...
      it('should produce different keys than TON type for same mnemonic', async () => {
        ...
        const resultBip39 = await MnemonicToKeyPair(
          VALID_TON_MNEMONIC_24,
          'bip39',
        );

        expect(
          Buffer.from(resultTon.publicKey).equals(
            Buffer.from(resultBip39.publicKey),
          ),
        ).toBe(false);
      });
    });
    ...
    describe('error handling', () => {
      it('should throw error for mnemonic with less than 12 words', async () => {
        ...
        await expect(MnemonicToKeyPair(shortMnemonic)).rejects.toThrow(
          WalletKitError,
        );
      ...
      it('should throw error for mnemonic with 15 words', async () => {
        ...
        await expect(MnemonicToKeyPair(invalidLengthMnemonic)).rejects.toThrow(
          WalletKitError,
        );
        await expect(
          MnemonicToKeyPair(invalidLengthMnemonic),
      ...
      it('should throw error for mnemonic with more than 24 words', async () => {
        ...
        await expect(MnemonicToKeyPair(longMnemonic)).rejects.toThrow(
          WalletKitError,
        );
      ...
      it('should include expected and received word count in error message', async () => {
        ...
        await expect(MnemonicToKeyPair(shortMnemonic)).rejects.toThrow(
          /expected 12 or 24 words, got 5/,
        );
      });

      it('should throw error for invalid mnemonic type', async () => {
        await expect(
          MnemonicToKeyPair(VALID_TON_MNEMONIC_24, 'invalid' as any),
        ).rejects.toThrow(WalletKitError);
      ...
      it('should include received type in error message for invalid type', async () => {
        await expect(
          MnemonicToKeyPair(VALID_TON_MNEMONIC_24, 'invalid' as any),
        ).rejects.toThrow(/expected "ton" or "bip39", got "invalid"/);
      });
    });
  });
