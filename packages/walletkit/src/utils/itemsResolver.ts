...
function resolveTonItem(
  item: StructuredItem & { type: 'ton' },
): TransactionRequestMessage {
  return {
...
async function resolveJettonItem(
  ...
  const customPayload = item.customPayload
    ...
    : null;
  const forwardPayload = item.forwardPayload
    ? Cell.fromBase64(item.forwardPayload)
    : null;
...
function resolveNftItem(
  item: StructuredItem & { type: 'nft' },
  wallet: Wallet,
): TransactionRequestMessage {
  const customPayload = item.customPayload
    ? Cell.fromBase64(item.customPayload)
