 ...
 */
export function storeNftTransferMessage(
 ...
 */
export function createNftTransferRawPayload(
  params: NftTransferRawParams,
): Cell {
  const transferMessage: NftTransferMessage = {
    queryId: BigInt(params.queryId),
    newOwner:
      typeof params.newOwner === 'string'
        ? Address.parse(params.newOwner)
        : params.newOwner,
...
export function createCommentPayloadBase64(comment: string): Base64String {
  return createCommentPayload(comment)
    .toBoc()
