...
import {
  ...
  validateNetwork,
  validateValidUntil,
} from '../handlers/transactionValidators';
import {
 ...
 */
export function getWalletFromEvent(
...
export function getClientForWallet(
  walletManager: WalletManager,
  event: { walletId?: string },
): ApiClient {
...
export function parseTonConnectTransactionRequest(
  ...
  try {
    ...
    const rawParams = JSON.parse(
      event.params[0],
    ) as RawConnectTransactionParamContent;
