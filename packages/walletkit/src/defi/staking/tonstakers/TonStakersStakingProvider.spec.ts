...
describe('TonStakersStakingProvider', () => {
  ...
  beforeEach(() => {
    ...
    buildUnstakeMessageSpy = vi
      .spyOn(PoolContract.prototype, 'buildUnstakeMessage')
      .mockResolvedValue({
        address: 'EQMockJettonWallet',
        amount: CONTRACT.UNSTAKE_FEE_RES.toString(),
        payload: 'mock-unstake-payload' as Base64String,
      });
    vi.spyOn(PoolContract.prototype, 'getPoolBalance').mockResolvedValue(
      500000000000n,
    );
    ...
    const factory = createTonstakersProvider({
      [Network.mainnet().chainId]: {
        metadata: {
          contractAddress:
            'EQCkWxfyhAkim3g2DjKQQg8T5P4g-Q1-K_jErGcDJZ4i-vqR' as UserFriendlyAddress,
        },
      },
    });
    provider = factory({
  ...
  describe('unstake mode flags', () => {
    it.each([
      ...
      {
        mode: UnstakeMode.ROUND_END,
        waitTillRoundEnd: true,
        fillOrKill: false,
      },
    ])(
      'should set correct flags for $mode mode',
      async ({ mode, waitTillRoundEnd, fillOrKill }) => {
        const amount = '1';
        const quote = await provider.getQuote({
          direction: 'unstake',
          amount,
  ...
  describe('getStakedBalance', () => {
    it('should return user balance and provider info', async () => {
      ...
      vi.spyOn(PoolContract.prototype, 'getStakedBalance').mockResolvedValue(
        '1000000000',
      ); // 1 tsTON

      const balance = await provider.getStakedBalance(
        testUserAddress,
        Network.mainnet(),
      );
  ...
  describe('getStakingProviderMetadata', () => {
    ...
    it('should use default receiveToken address from DEFAULT_METADATA for testnet', () => {
      ...
      const metadata = testnetProvider.getStakingProviderMetadata(
        Network.testnet(),
      );
      expect(metadata.receiveToken?.address).toBe(
    ...
    it('should prefer receiveToken from config metadata over the default', () => {
      ...
      const metadata = customProvider.getStakingProviderMetadata(
        Network.mainnet(),
      );
    ...
    it('should throw when metadata is not available for the network', () => {
      expect(() =>
        provider.getStakingProviderMetadata(Network.testnet()),
      ).toThrow();
    });
