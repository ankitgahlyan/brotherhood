...
export class StakingManager
  extends DefiManager<StakingProviderInterface>
  implements StakingAPI
{
  ...
  async getStakedBalance(
    ...
    try {
      return await this.getProvider(providerId).getStakedBalance(
        userAddress,
        network,
      );
    } catch (error) {
      log.error('Failed to get staking balance', {
        error,
        userAddress,
        network,
      });
