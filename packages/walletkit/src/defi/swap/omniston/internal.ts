...
type SdkSettlementMethodRecord = Record<
  StonFiSettlementMethod,
  StonFiSettlementMethod
>;
// On SDK drift, `SdkSettlementMethodLiteralsCheck` is `never` and the next line errors
type SdkSettlementMethodLiteralsCheck =
  SettlementMethodLiteralsBySdkKey extends SdkSettlementMethodRecord
    ? true
    : never;
