...
export class OmnistonSwapProvider extends SwapProvider<OmnistonProviderOptions> {
  ...
  async getQuote(
    params: SwapQuoteParams<OmnistonProviderOptions>,
  ): Promise<SwapQuote> {
    ...
    try {
      ...
      const referrerAddress =
        params.providerOptions?.referrerAddress ?? this.referrerAddress;
      const referrerFeeBps =
        params.providerOptions?.referrerFeeBps ?? this.referrerFeeBps;
      const flexibleReferrerFee =
        params.providerOptions?.flexibleReferrerFee ?? this.flexibleReferrerFee;
      ...
      const quoteRequest: QuoteRequest = {
        ...
        referrerAddress: referrerAddress
          ? toOmnistonAddress(
              Address.parse(referrerAddress).toString({ bounceable: true }),
              params.network,
            )
          : undefined,
      ...
      const quoteEvent = await new Promise<QuoteResponseEvent>(
        (resolve, reject) => {
          let isSettled = false;
          ...
          const unsubscribe = this.omniston
            .requestForQuote(quoteRequest)
            .subscribe({
              next: (event) => {
                ...
                if (event.type === 'noQuote') {
                  ...
                  reject(
                    new SwapError(
                      'No quote available for this swap',
                      SwapErrorCode.InsufficientLiquidity,
                    ),
                  );
                  return;
                }
  ...
  async buildSwapTransaction(params: SwapParams): Promise<TransactionRequest> {
    ...
    if (!metadata || !isOmnistonQuoteMetadata(metadata)) {
      throw new SwapError(
        'Invalid quote: missing Omniston quote data',
        SwapErrorCode.InvalidQuote,
      );
    }
    ...
    try {
      ...
      if (
        ...
        omnistonQuote.tradeStartDeadline < now
      ) {
        throw new SwapError(
      ...
      const omnistonUserAddress = toOmnistonAddress(
        userAddress,
      ...
      const destinationAddressRaw = params.destinationAddress
        ...
        : userAddress;
      const omnistonDestinationAddress = toOmnistonAddress(
        destinationAddressRaw,
        params.quote.network,
      );
      ...
      const buildResult = await withTimeout(
        this.omniston.buildTransfer(transactionRequest),
        this.buildTimeoutMs,
      );
      ...
      if (!messages || messages.length === 0) {
        throw new SwapError(
          'Failed to build transaction: no messages returned',
      ...
      const transaction: TransactionRequest = {
        ...
        messages: messages.map((message) => ({
          ...
          stateInit: message.jettonWalletStateInit
            ? asBase64(message.jettonWalletStateInit)
            : undefined,
        })),
  ...
  private mapOmnistonQuoteToSwapQuote(
    quote: Quote,
    ...
    return {
      ...
      expiresAt: quote.tradeStartDeadline
