...
export const tokenToMinter = (token: SwapToken): string => {
  ...
  return Address.parse(token.address).toString({
    bounceable: true,
    urlSafe: true,
  });
};
...
export const validateNetwork = (network: Network): void => {
  if (network.chainId !== Network.mainnet().chainId) {
    throw new Error(
      `DeDust only supports mainnet. Got chainId: ${network.chainId}`,
    );
  }
};
 ...
 */
export const isDeDustQuoteMetadata = (
  metadata: unknown,
): metadata is DeDustQuoteMetadata => {
