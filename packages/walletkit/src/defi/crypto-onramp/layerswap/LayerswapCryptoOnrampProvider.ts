 ...
 */
export class LayerswapCryptoOnrampProvider extends CryptoOnrampProvider<
  undefined,
...
> {
  ...
  constructor(config: LayerswapProviderConfig = {}) {
    ...
    this.supportedChains =
      config.supportedChains ?? DEFAULT_LAYERSWAP_SUPPORTED_CHAINS;
    this.destinationTokens =
      config.destinationTokens ?? LAYERSWAP_DESTINATION_TOKENS;
  }

  async getQuote(
    params: CryptoOnrampQuoteParams<undefined>,
  ): Promise<CryptoOnrampQuote<LayerswapQuoteMetadata>> {
    const { sourceCurrency, targetCurrency, recipientAddress, refundAddress } =
      params;
    ...
    if (
      ...
      throw new CryptoOnrampError(
        'Layerswap: refundAddress is not in the expected format (got "' +
          refundAddress +
          '")',
    ...
    const amountDecimal = formatBaseUnits(
      params.amount,
    ...
    const targetAmountBaseUnits = parseBaseUnits(
      data.quote.receive_amount,
  ...
  async createDeposit(
    params: CryptoOnrampDepositParams<LayerswapQuoteMetadata>,
  ...
  async getStatus(
    params: CryptoOnrampStatusParams,
  ): Promise<CryptoOnrampStatus> {
    ...
    const json = (await response.json().catch(() => undefined)) as
      LayerswapGetSwapResponse | undefined;
  ...
  async getSupportedCurrencies(): Promise<CryptoOnrampSupportedCurrencies> {
    ...
    const results = await Promise.allSettled(
      destination.map((dest) =>
        this.fetchSources(
          LAYERSWAP_DESTINATION_NETWORK,
          toLayerswapDestinationToken(dest.symbol),
        ),
      ),
    );
  ...
  private async fetchSources(
    ...
    const json = (await response.json().catch(() => undefined)) as
      { data?: LayerswapNetworkWithTokens[] } | undefined;
...
const buildSlugToCaip2Map = (
  caip2ToConfig: Record<string, LayerswapChainConfig>,
): Record<string, string> => {
  const reversed: Record<string, string> = {};
  for (const [caip2, config] of Object.entries(caip2ToConfig))
    reversed[config.slug] = caip2;
  return reversed;
};
