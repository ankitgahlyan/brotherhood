...
export class CryptoOnrampManager
  extends DefiManager<CryptoOnrampProviderInterface>
  implements CryptoOnrampAPI
{
  ...
  getMetadata(providerId?: string): CryptoOnrampProviderMetadata {
    ...
    log.debug('Getting crypto onramp metadata', {
  ...
  async createDeposit<TProviderOptions = unknown>(
    ...
    const selectedProviderId =
      providerId || params.quote?.providerId || this.defaultProviderId;
    ...
    try {
      const deposit =
        await this.getProvider(selectedProviderId).createDeposit(params);
  ...
  async getStatus(
    ...
    try {
      const status =
        await this.getProvider(selectedProviderId).getStatus(params);
      ...
      log.error('Failed to get crypto onramp deposit status', {
        error,
        params,
      });
  ...
  async getSupportedCurrencies(
    providerId?: string,
  ): Promise<CryptoOnrampSupportedCurrencies> {
    const selectedProviderId = providerId || this.defaultProviderId;
    log.debug('Discovering crypto onramp supported currencies', {
      providerId: selectedProviderId,
    });
    ...
    try {
      ...
      log.error('Failed to discover crypto onramp supported currencies', {
        error,
      });
      throw toDefiError(
        error,
        'Failed to discover crypto onramp supported currencies',
      );
    }
  }
}
