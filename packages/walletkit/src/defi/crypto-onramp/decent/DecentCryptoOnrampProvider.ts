 ...
 */
const DECENT_NATIVE_TOKEN_ADDRESS =
 ...
 */
export class DecentCryptoOnrampProvider extends CryptoOnrampProvider<
  DecentQuoteOptions,
  DecentQuoteMetadata
> {
  ...
  getMetadata() {
    return {
      ...
      refundAddressMode: 'required' as const,
    };
  }
  ...
  constructor(config: DecentProviderConfig) {
    ...
    this.supportedChains =
      config.supportedChains ?? DEFAULT_DECENT_SUPPORTED_CHAINS;
    this.supportedCurrencies =
      config.supportedCurrencies ?? DEFAULT_DECENT_SUPPORTED_CURRENCIES;
  }
  ...
  async getQuote(
    ...
    if (!isEvmAddress(sender)) {
      throw new CryptoOnrampError(
        'Decent: senderAddress must be a valid EVM address (got "' +
          sender +
          '")',
    ...
    const srcToken =
      sourceCurrency.address === 'native'
        ? DECENT_NATIVE_TOKEN_ADDRESS
        : sourceCurrency.address;
    const dstToken =
      targetCurrency.address === 'ton'
        ? DECENT_NATIVE_TOKEN_ADDRESS
    ...
    url.searchParams.set(
      'slippage',
      String(params.providerOptions?.slippageBps ?? DEFAULT_SLIPPAGE_BPS),
    );
    ...
    const body = (await response
      .json()
      .catch(() => undefined)) as DecentGetActionResponse;
  ...
  async createDeposit(
    params: CryptoOnrampDepositParams<DecentQuoteMetadata>,
    ...
    const needsRefetch =
      ...
      (params.refundAddress !== undefined &&
        params.refundAddress !== metadata.sender);
