...
const makeProvider = (providerId: string): GaslessProviderInterface => ({
  ...
  sendTransaction: vi
    .fn<(p: GaslessSendParams) => Promise<GaslessSendResponse>>()
    .mockResolvedValue({
...
describe('GaslessManager delegation', () => {
  it('forwards getMetadata to the default provider', async () => {
    ...
    await expect(manager.getMetadata()).resolves.toEqual({
      name: 'Test',
  ...
  it('rethrows provider errors from getQuote without wrapping', async () => {
    const { manager } = makeManager();
    const provider = makeProvider('one');
    const error = new GaslessError(
      'relayer down',
      GaslessErrorCode.QuoteFailed,
    );
    (provider.getQuote as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
      error,
    );
    manager.registerProvider(provider);
...
describe('GaslessManager events', () => {
  it('emits provider:registered when a new provider is added', () => {
    ...
    (
      emitter as unknown as EventEmitter<{
        'provider:registered': { providerId: string; type: string };
      }>
    ).on('provider:registered', handler);
  ...
  it('emits provider:default-changed when the default switches', () => {
    ...
    (
      emitter as unknown as EventEmitter<{
        'provider:default-changed': { providerId: string; type: string };
      }>
    ).on('provider:default-changed', handler);
