...
import {
  buildInternalMessageCell,
  hexBocToBase64,
  stripHexPrefix,
} from '../utils';
import type {
  TonApiGaslessEstimateRequest,
  TonApiGaslessEstimateResponse,
} from '../types/estimate';
 ...
 */
export const buildGaslessQuoteRequest = (
 ...
 */
export const mapGaslessQuote = (
  raw: TonApiGaslessEstimateResponse,
  network: Network,
): GaslessQuote => ({
  network,
  messages: raw.messages.map((message) => ({
    ...
    stateInit: message.state_init
      ? hexBocToBase64(message.state_init)
      : undefined,
  })),
  fee: raw.commission,
