...
const cellToBase64String = (cell: Cell): Base64String =>
  cell.toBoc().toString('base64') as Base64String;
...
describe('internalBocToExternalMessageBoc', () => {
  ...
  it('throws when input cannot be parsed as a relaxed message', () => {
    const garbage = cellToBase64String(
      beginCell().storeUint(0xdead, 16).endCell(),
    );
