...
const makeProvider = (
  ...
  options: Partial<Parameters<typeof createTonApiGaslessProvider>[0]> & {
    networks?: Network[];
  } = {},
): TonApiGaslessProvider => {
...
describe('TonApiGaslessProvider configuration', () => {
  ...
  it('throws when no networks are configured and no chains are passed', () => {
    expect(() =>
      TonApiGaslessProvider.createFromContext(ctxWith([]), {}),
    ).toThrow(/no eligible networks/);
  });
...
describe('TonApiGaslessProvider.getConfig', () => {
  ...
  it('hits /v2/gasless/config on the mainnet endpoint when called for mainnet', async () => {
    fetchApi.mockResolvedValueOnce(
      jsonResponse(rawConfig({ gas_jettons: [] })),
    );

    await provider.getConfig(Network.mainnet());

    expect((fetchApi.mock.calls[0][0] as URL).toString()).toBe(
      'https://tonapi.io/v2/gasless/config',
    );
  });
  ...
  it('uses the testnet endpoint when called for testnet', async () => {
    ...
    expect((fetchApi.mock.calls[0][0] as URL).origin).toBe(
  ...
  it('throws GaslessError(UNSUPPORTED_OPERATION) when called for a non-configured chain', async () => {
    const mainnetOnly = makeProvider(fetchApi, {
      networks: [Network.mainnet()],
    });

    await expect(
      mainnetOnly.getConfig(Network.testnet()),
...
describe('TonApiGaslessProvider.getQuote', () => {
  ...
  it('maps relayer response to GaslessQuote and threads network through', async () => {
    ...
    expect(() =>
      Cell.fromBase64(result.messages[0].payload as string),
    ).not.toThrow();
  });
  ...
  it('strips 0x prefix from walletPublicKey and serializes BoCs as hex', async () => {
    fetchApi.mockResolvedValueOnce(
      jsonResponse(
        makeRawQuote({ messages: [], commission: '0', valid_until: 0 }),
      ),
    );
  ...
  it('routes to the chain-specific endpoint based on params.network', async () => {
    const multiChain = makeProvider(fetchApi, {
      networks: [Network.mainnet(), Network.testnet()],
    });
    fetchApi.mockResolvedValueOnce(
      jsonResponse(
  ...
  it.skip('maps TonAPI error_code 40007 to GaslessError(FEE_ASSET_NOT_OWNED)', async () => {
    fetchApi.mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          error:
            'failed to resolve jetton master for jetton wallet 0:7742502c809ab1decdfcb82cd292b92a50be6ef823e6675ea541fce61bd1c88d',
          error_code: 40007,
        }),
...
describe('TonApiGaslessProvider.sendTransaction', () => {
  ...
  const validExternalHex = internalBocToExternalMessageBoc(
    buildSignedInternalBoc(),
  )
    .toBoc()
    .toString('hex');
  const successfulSendResponse = () =>
    jsonResponse({ protocol_name: 'tonapi', external: validExternalHex });
  ...
  it('parses the relayer-broadcast external BoC and returns a GaslessSendResponse', async () => {
    ...
    const externalCell = internalBocToExternalMessageBoc(
      buildSignedInternalBoc(),
    );
    const externalHex = externalCell.toBoc().toString('hex');
    fetchApi.mockResolvedValueOnce(
      jsonResponse({ protocol_name: 'tonapi', external: externalHex }),
    );
  ...
  it('throws GaslessError(SEND_FAILED) when `external` is not a parseable BoC (e.g. a bare hash)', async () => {
    ...
    fetchApi.mockResolvedValueOnce(
      jsonResponse({ protocol_name: 'tonapi', external: '00'.repeat(32) }),
    );

    await expect(
      provider.sendTransaction(baseSendParams),
    ).rejects.toMatchObject({
      name: 'GaslessError',
