...
import {
  buildGaslessQuoteRequest,
  mapGaslessQuote,
} from './mappers/map-gasless-quote';
import {
  buildGaslessSendRequest,
  mapGaslessSend,
} from './mappers/map-gasless-send';
import type {
...
export class TonApiGaslessProvider extends GaslessProvider {
   ...
   */
  private constructor(
    chainConfig: Record<string, TonApiGaslessChainConfig>,
    options: TonApiGaslessProviderConfig,
  ) {
    super();
    ...
    this.sendRetryDelayMs =
      options.sendRetryDelayMs ?? DEFAULT_SEND_RETRY_DELAY_MS;
    this.quoteRetries = options.quoteRetries ?? DEFAULT_QUOTE_RETRIES;
    this.quoteRetryDelayMs =
      options.quoteRetryDelayMs ?? DEFAULT_QUOTE_RETRY_DELAY_MS;
    this.configCacheTtlMs =
      options.configCacheTtlMs ?? DEFAULT_CONFIG_CACHE_TTL_MS;
    ...
    if (this.configCacheTtlMs > 0) {
      ...
      this.configCache = new LRUCache<string, GaslessConfig, Network>({
        ...
        fetchMethod: (_chainId, _stale, { context }) =>
          this.fetchConfig(context),
      });
    }
  ...
  static createFromContext(
    ...
    const configuredChains = new Set(
      ctx.networkManager.getConfiguredNetworks().map((n) => n.chainId),
    );
    ...
    if (config.chains) {
      for (const [chainId, perChain] of Object.entries(config.chains)) {
        if (!configuredChains.has(chainId)) {
          log.warn('Skipping TonApi gasless chain not configured in the kit', {
            chainId,
          });
          continue;
        }
  ...
  async getConfig(network: Network): Promise<GaslessConfig> {
    ...
    const value = await this.configCache.fetch(network.chainId, {
      context: network,
    });
    if (!value) {
      throw new GaslessError(
        'Gasless config cache returned no value',
  ...
  private async fetchConfig(network: Network): Promise<GaslessConfig> {
    try {
      ...
      log.error('Failed to fetch gasless config', {
        error,
      ...
      throw mapTonApiGaslessError(
        error,
        GaslessErrorCode.ConfigFailed,
        'Failed to fetch gasless config',
      );
    }
  }
  ...
  async getQuote(params: GaslessQuoteParams): Promise<GaslessQuote> {
    ...
    try {
      ...
      const raw = await CallForSuccess(
        () =>
          http.postJson<TonApiGaslessEstimateResponse>(
      ...
      throw mapTonApiGaslessError(
        ...
        'Failed to quote gasless transaction',
      );
    }
  }
...
export const createTonApiGaslessProvider = (
  ...
  return (ctx: ProviderFactoryContext) =>
    TonApiGaslessProvider.createFromContext(ctx, config);
};
