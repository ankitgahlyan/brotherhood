...
export function validateTransactionMessages(
  ...
  messages.forEach((msg, index) => {
    const msgErrors = validateTransactionMessage(
      msg,
      isTonConnect,
...
export function validateMessageObject(
  message: any,
  requireFriendlyAddress: boolean = true,
): ValidationResult {
  const errors: string[] = [];
  ...
  if (!message.address || typeof message.address !== 'string') {
    ...
    if (
      requireFriendlyAddress
        ? !isFriendlyTonAddress(message.address)
