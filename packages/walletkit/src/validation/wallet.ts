...
export function validatePublicKey(publicKey: string): ValidationResult {
  ...
  if (publicKey.length !== 64) {
    errors.push(
      `publicKey must be 64 characters long, got ${publicKey.length}`,
    );
  }
 ...
 */
export async function validateWalletMethods(
  wallet: Wallet,
  ...
  try {
    ...
    errors.push(
      `getAddress() failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  if (wallet.getStateInit) {
    try {
      ...
      errors.push(
        `getStateInit() failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
