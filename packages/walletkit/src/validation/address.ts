...
export function validateTonAddress(
  ...
  if (!allValid) {
    errors.push(
      'address must be in valid TON format (raw, bounceable, or non-bounceable)',
    );
...
export function validateRawAddress(address: string): ValidationResult {
  ...
  try {
    if (!Address.isRaw(address)) {
      errors.push(
        'raw address must be in format "workchain:account_id" where account_id is 64 hex chars',
      );
    }
  } catch {
    errors.push(
...
export function validateBouncableAddress(address: string): ValidationResult {
  ...
  try {
    ...
    errors.push(
      'bounceable address must be in format "EQ" or "UQ" followed by 46 base64url chars',
    );
  }
...
export function validateNonBouncableAddress(address: string): ValidationResult {
  ...
  try {
    ...
    errors.push(
      'non bounceable address must be in format "EQ" or "UQ" followed by 46 base64url chars',
    );
  }
  return {
    isValid: errors.length === 0,
...
export function detectAddressFormat(
  address: string,
): 'raw' | 'bounceable' | 'non-bounceable' | 'unknown' {
