...
class EnumNodeParserWithNames {
  ...
  createType(node) {
    ...
    members.forEach((member, index) => {
      ...
      const isHidden = jsDoc.some(
        (tag) => tag.tagName.text === 'hidden' || tag.tagName.text === 'ignore',
      );
      if (isHidden) return;
    ...
    const enumType = new EnumTypeWithNames(
      id,
      values,
      memberNames,
      annotations,
...
class ConstEnumNodeParser {
  ...
  supportsNode(node) {
    ...
    if (!typeNode || typeNode.kind !== ts.SyntaxKind.IndexedAccessType)
      return false;
    ...
    if (typeNode.indexType.operator !== ts.SyntaxKind.KeyOfKeyword)
      return false;
  ...
  createType(node) {
    ...
    const annotations =
      typeAliasName !== constName ? { 'x-definition-name': constName } : {};
  ...
  getDefinition(type) {
    ...
    if (type instanceof EnumTypeWithNames) {
      ...
      if (memberNames && memberNames.length === values.length) {
        definition['x-enum-varnames'] = memberNames.map((name) =>
      ...
      if (
        ...
        definition.format &&
        INTEGER_FORMATS.includes(definition.format)
      ) {
...
class AnnotatedTypeFormatterWithIntegers extends tsj.AnnotatedTypeFormatter {
  getDefinition(type) {
    ...
    if (
      def.type === 'number' &&
      def.format &&
      INTEGER_FORMATS.includes(def.format)
    ) {
      def.type = 'integer';
    }
...
class DiscriminatedUnionNodeParser {
  ...
  supportsNode(node) {
    ...
    const isDiscriminated = typeNode.types.every((member) =>
      this.isDiscriminatedMember(member),
    );
    if (!isDiscriminated) {
      return false;
    }
  ...
  createType(node, context) {
    ...
    for (const memberNode of typeNode.types) {
      ...
      if (!valuePropNode?.type || !this.isPrimitiveType(valuePropNode.type))
        continue;
      ...
      const isArrayRecursion =
        isRecursive && valuePropNode.type.kind === ts.SyntaxKind.ArrayType;
      ...
      if (isArrayRecursion) {
        ...
        valueType = new tsj.ArrayType(
          new tsj.DefinitionType(typeName, new tsj.AnyType()),
        );
        ...
        valueType = this.childNodeParser.createType(
          valuePropNode.type,
          context,
        );
      }
  ...
  isPrimitiveType(typeNode) {
    ...
    if (typeNode.kind === ts.SyntaxKind.TypeLiteral) {
      ...
      return (
        members.length === 1 && members[0].kind === ts.SyntaxKind.IndexSignature
      );
    }
    return false;
  }
...
class DiscriminatedUnionTypeFormatter {
  ...
  getAssociatedValueType(type) {
    const derefed = this.derefType(type);
    if (derefed instanceof tsj.ObjectType) {
      const valueProp = derefed
        .getProperties()
        .find((p) => p.getName() === 'value');
  ...
  getDefinition(type) {
    const union =
      type instanceof DiscriminatedUnionType ? type.getInnerUnion() : type;
    const caseValueRefs =
      type instanceof DiscriminatedUnionType
        ? type.getCaseValueRefs()
        : new Map();
    const syntheticTypes =
      type instanceof DiscriminatedUnionType ? type.getSyntheticTypes() : [];
  ...
  createType(node, context) {
    ...
    for (const member of node.members) {
      if (member.kind === ts.SyntaxKind.PropertySignature) {
        ...
        if (member.type) {
          ...
          if (typeParamNames.has(typeText)) {
            ...
            if (
              ...
              member.type.typeArguments.every((arg) =>
                typeParamNames.has(arg.getText()),
              )
            ) {
              const baseName = member.type.typeName.getText();
              const argList = member.type.typeArguments
                .map((arg) => arg.getText())
                .join(', ');
        ...
        const formatTag = jsDocTags.find(
          (tag) => tag.tagName.text === 'format',
        );
    ...
    const innerType = new GenericPropertiesObjectType(
      interfaceName,
      properties,
    );

    // Wrap in GenericInterfaceType
    const genericType = new GenericInterfaceType(
      interfaceName,
      innerType,
      typeParameters,
    );
...
class InlineTypeLiteralAnnotationParser {
  ...
  extractFormat(propertyNode) {
    ...
    const m = trivia.match(
      /\/\*\*[\s\S]*?@format\s+([A-Za-z0-9_-]+)[\s\S]*?\*\//,
    );
    return m ? m[1] : null;
  }
...
class OpenAPIReferenceTypeFormatter {
  ...
  getChildren(type) {
    ...
    return this.childTypeFormatter.getChildren(
      new tsj.DefinitionType(type.getName(), type.getType()),
    );
  }
}
...
class AllTypesSchemaGenerator extends tsj.SchemaGenerator {
  ...
  createSchemaFromNodes(rootNodes) {
    ...
    for (const rootNode of rootNodes) {
      try {
        const rootType = this.nodeParser.createType(
          rootNode,
          new tsj.Context(),
        );
  ...
  appendRootChildDefinitions(rootType, childDefinitions) {
    ...
    for (const child of children) {
      ...
      if (!(name in childDefinitions)) {
        childDefinitions[name] = this.typeFormatter.getDefinition(
          child.getType(),
        );
      }
    }
 ...
 */
function processDiscriminatorMemberTypes(
  cases,
  discriminatorField,
  definitions,
) {
  for (const { rawValue, ref } of cases) {
    ...
    const constantValue =
      discProp.const || (discProp.enum && discProp.enum[0]) || rawValue;
    ...
    if (memberDef.required) {
      memberDef.required = memberDef.required.filter(
        (r) => r !== discriminatorField,
      );
...
function isAnyOfDiscriminatedUnion(propDef) {
  if (!propDef || !Array.isArray(propDef.anyOf) || propDef.anyOf.length < 2)
    return false;
  return propDef.anyOf.every(
    (variant) => getInlineVariantDiscriminator(variant) !== null,
  );
}
  ...
  for (const [parentName, typeDef] of Object.entries(definitions)) {
    ...
    if (topLevel) {
      ...
      if (singleFieldCodingKeys.size > 0) {
        typeDef['x-single-field-coding-keys'] = [...singleFieldCodingKeys].map(
          (k) => ({ name: k }),
        );
      }
      continue;
  ...
  for (const [, def] of Object.entries(definitions)) {
    ...
    for (const [, caseProp] of Object.entries(def.properties || {})) {
      ...
      if (caseProp['x-single-field-variant']) {
        ...
        !caseProp['x-empty-variant'] &&
        caseProp.allOf &&
        caseProp.allOf[0] &&
        caseProp.allOf[0].$ref
      ) {
...
function valueSchemaShape(caseProp) {
  ...
  if (caseProp.type === 'string') {
    ...
    const flags =
      caseProp['x-int-format-flags'] ||
      (caseProp.format ? { [caseProp.format]: true } : null);
    if (flags) {
  ...
  const classifyEntry = (entry) => {
    ...
    if (entry.type === 'string' && !entry.$ref) {
      if (typeof entry.const === 'string')
        return { kind: 'literal', rawValue: entry.const };
    ...
    if (typeof entry.$ref === 'string' && Object.keys(entry).length === 1) {
      ...
      if (target.type === 'string' && !target.$ref) {
        if (
          ...
          target.enum.length === 1 &&
          typeof target.enum[0] === 'string'
        ) {
          return {
            kind: 'literal',
            rawValue: target.enum[0],
            synthetic: targetName,
          };
        }
...
function postProcessDefinitionNames(schema) {
  ...
  for (const [oldName, newName] of Object.entries(renames)) {
    ...
    oldToNewRef[`#/components/schemas/${oldName}`] =
...
function postProcessTypeNameCasing(schema) {
  ...
  for (const [oldName, newName] of Object.entries(renames)) {
    ...
    refMap[`#/components/schemas/${oldName}`] =
      `#/components/schemas/${newName}`;
  }
...
if (!inputPath || !outputPath) {
  console.error(
    'Usage: node generate-json-schema.js <input-path> <output-path>',
  );
  process.exit(1);
}
...
try {
  ...
  const formatter = tsj.createFormatter(
    config,
    (fmt, circularReferenceTypeFormatter) => {
      fmt.addTypeFormatter(
        new OpenAPIDefinitionTypeFormatter(circularReferenceTypeFormatter),
      );
      fmt.addTypeFormatter(
        new OpenAPIReferenceTypeFormatter(circularReferenceTypeFormatter),
      );
      fmt.addTypeFormatter(
        new AnnotatedTypeFormatterWithIntegers(circularReferenceTypeFormatter),
      );
      fmt.addTypeFormatter(new EnumTypeFormatterWithVarnames());
      fmt.addTypeFormatter(
        new SyntheticValueTypeFormatter(circularReferenceTypeFormatter),
      );
      fmt.addTypeFormatter(
        new DiscriminatedUnionTypeFormatter(circularReferenceTypeFormatter),
      );
      fmt.addTypeFormatter(
        new GenericInterfaceTypeFormatter(circularReferenceTypeFormatter),
      );
      fmt.addTypeFormatter(
        new GenericPropertiesObjectTypeFormatter(
  ...
  const generator = new AllTypesSchemaGenerator(
    program,
    parser,
    formatter,
    config,
