...
export type RawStructuredItem =
  RawTonTransferItem | RawJettonTransferItem | RawNftTransferItem;
  ...
  switch (raw.type) {
    ...
    case 'jetton':
      return {
        ...
        customPayload: raw.customPayload
          ? asBase64(raw.customPayload)
          : undefined,
        forwardAmount: raw.forwardAmount,
        forwardPayload: raw.forwardPayload
          ? asBase64(raw.forwardPayload)
          : undefined,
      };
    case 'nft':
      return {
...
export function toExtraCurrencies(
  extraCurrency: ConnectExtraCurrency | undefined,
): ExtraCurrencies | undefined {
  if (!extraCurrency) {
    return undefined;
  }
 ...
 */
export function parseConnectSignDataParamContent(
  event: RawBridgeEventSignData,
): SignDataPayload | undefined {
  try {
    ...
    if (parsed.type === 'text') {
      ...
      signData = {
        type: 'binary',
        value: { content: parsed.bytes as Base64String },
      };
    } else if (parsed.type === 'cell') {
      signData = {
        type: 'cell',
        value: { schema: parsed.schema, content: parsed.cell as Base64String },
      };
    } else {
      return undefined;
    }
...
export function toTransactionRequestMessage(
...
export function toConnectTransactionParamMessage(
 ...
 */
export function toConnectTransactionParamContent(
...
export interface EventHandler<
  T extends BridgeEvent = BridgeEvent,
  V extends RawBridgeEvent = RawBridgeEvent,
> {
