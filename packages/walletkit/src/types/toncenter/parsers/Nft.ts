...
export function parseNftActions(
  ...
  for (const key of Object.keys(txs)) {
    ...
    for (const out of tx.out_msgs || []) {
      ...
      if (decoded?.['@type'] === 'nft_transfer') {
        ...
        const nftAddr = out.destination
          ? asAddressFriendly(out.destination)
          : '';
        const status = computeStatus(tx);
        const base = collectBaseTransactionsForSent(
          item,
          ownerFriendly,
          newOwner,
          nftAddr,
        );
        const action = buildNftAction(
          status,
          ownerFriendly,
          newOwner,
          nftAddr,
  ...
  for (const key of Object.keys(txs)) {
    ...
    if (
      ...
      const prevOwner =
        toAddr(getProp(decoded, 'prev_owner')) ||
        toAddr(getProp(decoded, 'old_owner'));
      const nftAddr = tx.in_msg?.source
      ...
      const base = collectBaseTransactionsForReceived(
        item,
        ownerFriendly,
        nftAddr,
      );
...
function collectBaseTransactionsForSent(
  ...
  return [ownerTonFromNft, assignToNewOwner, nftTransferHash].filter(
    Boolean,
  ) as Hex[];
}
...
function getNftType(tx: ToncenterTransaction): string | '' {
  ...
  return matchOpWithMap(
    t,
    ['nft_transfer', 'nft_ownership_assigned', 'nft_owner_changed', 'excess'],
    {
...
function collectBaseTransactionsForReceived(
  item: ToncenterTraceItem,
  ownerFriendly: string,
  nftAddr: string,
): Hex[] {
  ...
  for (const h of order) {
    ...
    if (acc !== ownerFriendly) {
      ...
      const targetsOwner = (tx.out_msgs || []).some(
        (m) => asAddressFriendly(m.destination) === ownerFriendly,
      );
...
function findTransactionByAccount(
  item: ToncenterTraceItem,
  account: string,
