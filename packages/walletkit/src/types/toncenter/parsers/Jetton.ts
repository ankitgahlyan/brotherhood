...
export function parseJettonActions(
  ...
  for (const key of Object.keys(txs)) {
    ...
    if (
      ...
      const comment =
        extractCommentFromDecoded(getForwardPayloadValue(decoded)) ?? undefined;
      ...
      const id = (findFirstOwnerTxId(ownerFriendly, item) ??
        Base64ToHex(tx.hash)) as Hex;
      ...
      const action: JettonTransferAction = {
        ...
        simplePreview: jettonPreview(
          ...
          [
            ...
            toContractAccount(
              jetton.address ||
                inferMinterFromAddressBook(addressBook)?.address ||
                '',
              addressBook,
            ),
          ],
...
function toAddr(raw?: unknown): string {
  ...
  if (isRecord(raw)) {
    ...
    if (
      (typeof wc === 'string' || typeof wc === 'number') &&
      typeof addr === 'string'
    ) {
...
function computeStatus(tx: ToncenterTransaction): StatusAction {
  ...
  const computePh = (tx.description as unknown as Record<string, unknown>)?.[
    'compute_ph'
  ] as Record<string, unknown> | undefined;
  const action = (tx.description as unknown as Record<string, unknown>)?.[
    'action'
  ] as Record<string, unknown> | undefined;
...
function findRecipientJettonWalletFromOut(
  tx: ToncenterTransaction,
): string | null {
  for (const m of tx.out_msgs || []) {
    const d = getDecoded(m) as Record<string, unknown> | null;
    if (
      m.opcode === OpCode.JettonInternalTransfer ||
      (d && d['@type'] === 'jetton_internal_transfer')
    ) {
...
function collectBaseTransactionsSent(
  item: ToncenterTraceItem,
  ownerFriendly: string,
): Hex[] {
...
function collectBaseTransactionsReceived(
  item: ToncenterTraceItem,
  ownerFriendly: string,
): Hex[] {
  const order = item.transactions_order || [];
  const findTx = (
    predicate: (tx: ToncenterTransaction) => boolean,
  ): Hex | null => {
    for (const h of order) {
      const tx = item.transactions[h];
      if (!tx) continue;
  ...
  const isType = (tx: ToncenterTransaction, type: string) =>
    getTxType(tx) === type;
  const jt = findTx((tx) => isType(tx, 'jetton_transfer'));
  const internal = findTx(
    (tx) =>
      isType(tx, 'jetton_internal_transfer') &&
      asAddressFriendly(tx.account) !== ownerFriendly,
  );
...
function buildJettonInfo(
  ...
  if (walletInfo?.jettonWallet?.jettonMaster) {
    const masterAddress = asMaybeAddressFriendly(
      walletInfo.jettonWallet.jettonMaster,
    );
  ...
  const metadata = (item as unknown as { metadata?: Record<string, unknown> })
    .metadata;
...
function inferMinterFromAddressBook(addressBook: AddressBook): {
  ...
  for (const key of Object.keys(addressBook)) {
    ...
    if (
      domain === knownMinterByDomain ||
      (typeof domain === 'string' && domain.includes('minter'))
    ) {
      return {
        address: key,
        name: 'Tether USD',
