...
export type SmartContractAction =
  SmartContractExecAction | SmartContractDeployAction;
...
export function parseContractActions(
  ...
  for (const hash of Object.keys(transactions)) {
    ...
    for (const msg of tx.out_msgs || []) {
      ...
      const baseTx: Hex = child
        ? Base64ToHex(child.hash)
        : Base64ToHex(tx.hash);
      ...
      const exec: SmartContractExecAction = {
        ...
        simplePreview: {
          ...
          accounts: [
            ...
            toContractAccount(contractAddress, addressBook),
          ],
        },
...
function isDeploy(child: ToncenterTransaction, msg: EmulationMessage): boolean {
  const created =
    child.orig_status === 'nonexist' && child.end_status === 'active';
  const hasInit =
    Boolean(msg.init_state) || Boolean(child.in_msg && child.in_msg.init_state);
