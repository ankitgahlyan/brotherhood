...
import {
  computeStatus,
  parseIncomingTonTransfers,
  parseOutgoingTonTransfers,
} from './parsers/TonTransfer';
  ...
  for (const [address, bookRow] of Object.entries(data.address_book)) {
    ...
    if (bookRow.interfaces && Array.isArray(bookRow.interfaces)) {
      const hasWalletInterface = bookRow.interfaces.some(
        (iface) =>
          typeof iface === 'string' && iface.toLowerCase().includes('wallet'),
      );
  ...
  for (const [address, meta] of Object.entries(data.metadata)) {
    ...
    for (const tokenInfo of meta.token_info) {
      if (tokenInfo.type === 'jetton_masters') {
        ...
        const decimals = masterInfo.extra?.decimals
          ? parseInt(masterInfo.extra.decimals, 10)
          : 0;
        const image =
          masterInfo.image ||
...
function buildEvent(
  ...
  const extHash = (() => {
    ...
    const inMsg = firstTx?.in_msg as
      { hash_norm?: string; hash?: string } | undefined;
    return (
      inMsg?.hash_norm ??
      inMsg?.hash ??
      data.external_hash ??
...
export function emulationEvent(
  data: ToncenterEmulationResponse,
  account?: string,
): Event {
  // Build a ToncenterTraceItem from emulation response
  const txEntries = Object.entries(data.transactions) as [
    string,
    ToncenterTransaction,
  ][];
  const byLtAsc = [...txEntries].sort((a, b) =>
    BigInt(a[1].lt) < BigInt(b[1].lt) ? -1 : 1,
  );
  ...
  const start_utime =
    byLtAsc.length > 0
      ? Math.min(...byLtAsc.map(([, tx]) => tx.now))
      : Math.floor(Date.now() / 1000);
  const end_utime =
    byLtAsc.length > 0
      ? Math.max(...byLtAsc.map(([, tx]) => tx.now))
      : start_utime;
  const mcSeqnos = byLtAsc.map(([, tx]) => tx.mc_block_seqno);
  const mc_seqno_start =
    mcSeqnos.length > 0 ? String(Math.min(...mcSeqnos)) : '0';
  const mc_seqno_end =
    mcSeqnos.length > 0 ? String(Math.max(...mcSeqnos)) : '0';
  const trace_id = transactions_order[0] ?? '';
  const rootTx = trace_id ? data.transactions[trace_id] : undefined;
  const external_hash =
    ((rootTx?.in_msg as unknown as { hash_norm?: string })?.hash_norm as
      string | undefined) ||
    ((rootTx?.in_msg as unknown as { hash?: string })?.hash as
      string | undefined) ||
    '';
  ...
  const traceItem: ToncenterTraceItem = {
    ...
    trace_info: {
      ...
      messages: byLtAsc.reduce(
        (sum, [, tx]) => sum + (tx.in_msg ? 1 : 0) + (tx.out_msgs?.length ?? 0),
        0,
      ),
      pending_messages: 0,
  ...
  (traceItem as unknown as { metadata?: Record<string, unknown> }).metadata =
  ...
  let inferredAccount =
    account && String(account).trim() ? String(account).trim() : undefined;
  if (!inferredAccount) {
  ...
  if (!inferredAccount) {
    inferredAccount =
      byLtAsc[0]?.[1]?.account ??
      (Object.values(data.transactions || {})[0]?.account as
        string | undefined);
  }
  if (!inferredAccount) {
