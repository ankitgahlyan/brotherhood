...
export function createWalletInitConfigLedger(
  params: WalletInitConfigLedgerInterface,
): WalletInitConfigLedgerInterface {
  return {
...
export class WalletV4R2LedgerAdapter implements WalletAdapter {
  ...
  async getSignedSendTransaction(
    ...
    try {
      ...
      const signedCell = await tonTransport.signTransaction(
        this.derivationPath,
        {
  ...
  async getSignedTonProof(input: ProofMessage): Promise<Hex> {
    ...
    try {
      ...
      const { signature } = await tonTransport.getAddressProof(
        this.derivationPath,
        {
