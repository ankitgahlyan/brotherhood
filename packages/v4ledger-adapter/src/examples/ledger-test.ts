  ...
  try {
    ...
    const ledgerConfig = createWalletInitConfigLedger({
      createTransport: async () =>
        (await TransportNodeHid.create()) as unknown as Transport,
      path,
...
async function testLedgerWallet() {
  try {
    ...
    logInfo(
      '⚠️  Make sure your Ledger device is connected and the TON app is open!',
    );
    ...
    logInfo(
      '📊 Wallet info:',
      util.inspect(walletInfo, { colors: true, depth: 3 }),
    );
    ...
    if (isTestSend) {
      ...
      logInfo(
        'ℹ️  Set TEST_SEND=true environment variable to actually send a transaction',
      );
    ...
    if (
      ...
      error instanceof Error &&
      error.message?.includes('App does not seem to be open')
    ) {
      logError(
        '❌ TON app is not open on Ledger device. Please open the TON app and try again.',
      );
    } else {
