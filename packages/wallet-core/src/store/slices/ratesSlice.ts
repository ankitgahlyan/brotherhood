...
const RATES_ENDPOINT =
  'https://api.dyor.io/v1/jettons?sort=liquidityUsd&order=desc&currency=ton&limit=100';
const RAW_GRAM_ADDRESS =
...
export const createRatesSlice: RatesSliceCreator = (set: SetState, get) => ({
  ...
  loadRates: async () => {
    ...
    if (isLoading) return;
    if (!error && lastUpdated > 0 && Date.now() - lastUpdated < RATES_TTL_MS)
      return;
