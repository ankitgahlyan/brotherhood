...
const pickDefaultFeeAsset = (
  supportedAssets: { address: string }[],
): string | null => {
  if (supportedAssets.length === 0) return null;
  const usdt = supportedAssets.find((asset) =>
      ...
      try {
        ...
        set((state) => {
          ...
          if (!state.gasless.feeAsset) {
            state.gasless.feeAsset = pickDefaultFeeAsset(
        ...
        set((state) => {
          ...
          state.gasless.error =
            error instanceof Error
              ? error.message
              : 'Failed to load gasless config';
        });
      }
    },

    getGaslessQuote: async ({
      recipientAddress,
      jettonAddress,
      transferAmount,
      comment,
      ...
      try {
        ...
        const jettonWalletAddress = await wallet.getJettonWalletAddress(
          asAddressFriendly(jettonAddress),
        );
