...
export const createStakingSlice: StakingSliceCreator = (
  set: SetState,
  get,
) => ({
  staking: {
  ...
  stake: async (params: Omit<StakeParams, 'userAddress'>) => {
    ...
    try {
      const transaction =
        await state.walletCore.walletKit.staking.buildStakeTransaction(
          {
            ...params,
            userAddress,
          },
  ...
  loadStakingData: async (userAddress: string) => {
    ...
    try {
      const [balance, info] = await Promise.all([
        state.walletCore.walletKit.staking.getStakedBalance(
          userAddress,
          network,
          providerId,
        ),
        state.walletCore.walletKit.staking.getStakingProviderInfo(
          network,
          providerId,
        ),
      ]);
