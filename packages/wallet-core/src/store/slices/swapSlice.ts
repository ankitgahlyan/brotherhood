...
export const createSwapSlice: SwapSliceCreator = (set: SetState, get) => ({
  ...
  validateSwapInputs: () => {
    ...
    if (fromToken.address === 'ton') {
      ...
      if (
        amountBigInt >
        tonBalance - parseUnits(NATIVE_GAS_RESERVE, fromToken.decimals)
      ) {
  ...
  getSwapQuote: async () => {
    ...
    try {
      ...
      const quote = await state.walletCore.walletKit.swap.getQuote(
        quoteParams,
        providerId,
      );
  ...
  executeSwap: async () => {
    ...
    try {
      ...
      const transaction =
        await state.walletCore.walletKit.swap.buildSwapTransaction({
          quote: currentQuote,
