...
export const ModalTitle: React.FC<ComponentProps<'h2'>> = ({
  className,
  ...props
}) => {
  const isMobile = useIsMobile();
  ...
  return (
    <Wrapper
      className={cn('text-xl font-bold text-foreground truncate', className)}
      {...props}
    />
  );
};

export const ModalBody: React.FC<ComponentProps<'div'>> = ({
  children,
  className,
  ...props
}) => (
...
export const ModalFooter: React.FC<ComponentProps<'div'>> = ({
  children,
  className,
  ...props
}) => (
