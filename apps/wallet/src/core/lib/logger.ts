  ...
  try {
    const storedLevel =
      typeof localStorage !== 'undefined'
        ? localStorage.getItem('wallet-log-level')
        : null;
    if (storedLevel !== null) {
...
export const createComponentLogger = (component: string) => {
  ...
  return {
    debug: (message: string, ...args: any[]) =>
      componentLogger.debug(message, ...args),
    info: (message: string, ...args: any[]) =>
      componentLogger.info(message, ...args),
    success: (message: string, ...args: any[]) =>
      componentLogger.success(message, ...args),
    warn: (message: string, ...args: any[]) =>
      componentLogger.warn(message, ...args),
    error: (message: string, error?: Error | any, ...args: any[]) => {
      if (error instanceof Error) {
...
export const setLogLevel = (level: number): void => {
  if (level >= 0 && level <= 5) {
    ...
    log.error(
      'Invalid log level. Must be between 0-5 (0=silent, 1=fatal, 2=error, 3=warn, 4=info, 5=debug)',
    );
  }
};
