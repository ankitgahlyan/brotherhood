...
export async function scanMnemonicWallets(params: {
  ...
  const [states, v5TestnetEvents, v5MainnetEvents, v4Events] =
    await Promise.all([
      client
        .getAccountStates([v5TestnetAddress, v5MainnetAddress, v4Address])
        .catch(() => null),
      client
        .getEvents({ account: v5TestnetAddress, limit: 1 })
        .catch(() => null),
      client
        .getEvents({ account: v5MainnetAddress, limit: 1 })
        .catch(() => null),
  ...
  return [
    {
      ...
      hasActivity:
        v5TestnetBal > 0n ||
        states?.[v5TestnetAddress]?.status === 'active' ||
        v5TestnetEvCount > 0,
    ...
    },
    {
      ...
      hasActivity:
        v5MainnetBal > 0n ||
        states?.[v5MainnetAddress]?.status === 'active' ||
        v5MainnetEvCount > 0,
    ...
    },
    {
      ...
      hasActivity:
        v4Bal > 0n || states?.[v4Address]?.status === 'active' || v4EvCount > 0,
