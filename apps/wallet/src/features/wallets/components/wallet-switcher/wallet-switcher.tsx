...
interface WalletSwitcherProps {
  ...
  onUpdateWalletType?: (
    walletId: string,
    newType: 'signer' | 'mnemonic' | 'ledger',
  ) => void;
  compact?: boolean;
}
...
export const WalletSwitcher: React.FC<WalletSwitcherProps> = ({
  ...
  const [showDetailsWalletId, setShowDetailsWalletId] = useState<string | null>(
    null,
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            className={`text-left min-w-0 flex-1 ${compact ? 'truncate' : ''}`}
          >
