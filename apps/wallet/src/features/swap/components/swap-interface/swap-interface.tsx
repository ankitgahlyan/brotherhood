...
export const SwapInterface: FC<SwapInterfaceProps> = ({ className }) => {
  ...
  const getTokenView = (token: SwapToken): TokenView => {
    ...
    return {
      symbol:
        token.symbol ||
        (jetton ? getJettonsSymbol(jetton) : undefined) ||
        'Token',
      icon: token.image ?? (jetton ? getJettonsImage(jetton) : undefined),
      balance: jetton?.balance
        ? formatUnits(jetton.balance, token.decimals)
        : '0',
    };
  };
  ...
  const handleExecuteSwap = async () => {
    ...
    if (ok)
      navigate('/wallet', {
        state: { message: `${fromView.symbol} sent successfully!` },
      });
  };
  ...
  return (
    <div className={cn('space-y-5', className)}>
      ...
      <div className="space-y-2">
        ...
        {useCustomDestination && (
          <Input.Container>
            ...
            <Input.Caption>
              Swapped tokens will be sent here instead of your wallet.
            </Input.Caption>
          </Input.Container>
        )}
      </div>
      ...
      {error && (
        <p className="rounded-2xl bg-red-50 p-3 text-center text-sm text-red-500">
          {error}
        </p>
      )}
