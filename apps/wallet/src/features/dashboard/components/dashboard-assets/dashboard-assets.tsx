...
export const DashboardAssets: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void Promise.all(
      DEFAULT_JETTONS.map((def) =>
        walletKit.jettons.getJettonInfo(def.address, network).catch(() => null),
      ),
    ).then((infos) => {
      if (cancelled) return;
  ...
  const selected = useMemo<AssetRowData[]>(() => {
    ...
    const heldByAddress = new Map(
      userJettons.map((jetton) => [jetton.address, jetton]),
    );
    ...
    for (const def of DEFAULT_JETTONS) {
      ...
      padded.push({
        ...
        icon: held
          ? imageSources(
              tokenImageUrls(held.info?.image),
              held.info?.image?.data,
            )
          : imageSources(
              info?.image ? [info.image] : undefined,
              info?.image_data,
            ),
