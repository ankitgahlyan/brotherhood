...
import { ActVoteProposal } from '@wrappers/FossFiWallet.gen';
...
export function useVoteProposal({
  ...
  const vote = useCallback(async () => {
    if (!walletAddress || !daoAddress || !proposalId)
      throw new Error('Missing parameter');
    const ownerAddr = Address.parse(walletAddress);
    const fiWalletAddr = await getFiWalletAddress(ownerAddr, network);
    const pollAddr = Address.parse(daoAddress);
  ...
  const isDisabled =
    !wallet || !walletAddress || !daoAddress || !proposalId || isSending;
