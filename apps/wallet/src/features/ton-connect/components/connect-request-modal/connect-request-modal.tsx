...
export const ConnectRequestModal: React.FC<ConnectRequestModalProps> = ({
  ...
  const walletName = (kitId: string): string =>
    savedByKitId.get(kitId)?.name ?? 'Wallet';

  // Saved-wallet records for the connectable wallets, in availableWallets order.
  const pickerWallets = useMemo(
    () =>
      availableWallets
        .map((w) => savedByKitId.get(w.getWalletId()))
        .filter((w): w is SavedWallet => Boolean(w)),
  ...
  const selectedSavedId = selectedWallet
    ? savedByKitId.get(selectedWallet.getWalletId())?.id
    : undefined;
  ...
  const handlePick = (savedId: string) => {
    ...
    const wallet = kitId
      ? availableWallets.find((w) => w.getWalletId() === kitId)
      : undefined;
  ...
  const walletSlot = (
    <WalletPlate
      name={
        selectedWallet ? walletName(selectedWallet.getWalletId()) : 'Wallet'
      }
