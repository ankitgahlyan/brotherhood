...
const renderDataToSign = (
  data: SignDataRequestEvent['preview']['data'],
): React.ReactNode => {
  switch (data.type) {
    case 'text':
      return (
...
export const SignDataRequestModal: React.FC<SignDataRequestModalProps> = ({
  ...
  const currentWallet = useMemo(
    () =>
      savedWallets.find((wallet) => wallet.kitWalletId === request.walletId) ??
      null,
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast.error('Failed to sign data', {
