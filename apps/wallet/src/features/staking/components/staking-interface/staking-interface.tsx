...
const UNSTAKE_MODES = [
  {
    ...
    label: 'Instant',
    hint: 'Receive GRAM immediately',
  },
  {
    mode: UnstakeMode.WHEN_AVAILABLE,
    label: 'When available',
    hint: 'Immediate if liquid, or up to ~18h queue',
  },
  {
    mode: UnstakeMode.ROUND_END,
    label: 'Round end',
    hint: 'Wait for cycle end (~18h) for best rate',
  },
];
...
export const StakingInterface: FC = () => {
  ...
  const handleAction = async () => {
    ...
    if (ok)
      navigate('/wallet', {
        state: { message: `${isStake ? 'Staked' : 'Unstaked'} successfully!` },
      });
  };
  ...
  const canPreview = Boolean(
    !validationError && !balanceError && amountNumber > 0,
  );
