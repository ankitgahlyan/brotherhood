...
export async function runSendTransactionTest(
  ...
  await expect(app.getByTestId('sendTransactionValidation')).toHaveText(
    'Validation Passed',
  );
}
...
export async function runSignDataTest(
  ...
  await expect(app.getByTestId('signDataValidation')).toHaveText(
    'Validation Passed',
  );
}
...
export async function runConnectTest(
  ...
  if (isExtension) {
    ...
    const connectUrl = await widget.connectUrl(
      await app.getByTestId('connect-button'),
    );
  ...
  await expect(app.getByTestId('connectValidation')).toHaveText(
    'Validation Passed',
    { timeout: 1 },
  );
}
