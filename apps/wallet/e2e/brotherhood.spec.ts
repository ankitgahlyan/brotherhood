...
test.describe('BrotherHood (FossFi) Feature UI', () => {
  test('should render BrotherHood screen and navigate tabs', async ({
    wallet,
  }) => {
  ...
  test('should validate form inputs on BrotherHood transfer form', async ({
    wallet,
  }) => {
    await suite('BrotherHood Features');
    ...
    await app
      .getByTestId('brotherhood-transfer-recipient')
      .fill(
  ...
  test('should trigger transaction modal when submitting FI transfer', async ({
    wallet,
  }) => {
    await suite('BrotherHood Features');
