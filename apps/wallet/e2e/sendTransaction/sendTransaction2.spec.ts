...
test('[amount] Error if as a number @allureId(2231)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[amount] Error if insufficient balance @allureId(2245)', async ({
  wallet,
  app,
  widget,
}) => {
...
test("[amount] Success if '0' @allureId(2261)", async ({
  wallet,
  app,
  widget,
}) => {
...
test('[amount] Success if as a string @allureId(2223)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[payload] Success if valid value @allureId(2253)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[stateInit] Error if invalid value @allureId(2248)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[stateInit] Success if absent @allureId(2233)', async ({
  wallet,
  app,
  widget,
}) => {
