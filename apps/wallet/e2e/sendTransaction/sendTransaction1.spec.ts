...
test('[address] Error if absent @allureId(2221)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[address] Error if in HEX format @allureId(2244)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[address] Error if invalid value @allureId(2230)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[address] Success if in bounceable format @allureId(2226)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[address] Success if in non-bounceable format @allureId(2227)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[amount] Error if absent @allureId(2247)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('Minting Jetton with Deployed Contract @allureId(1898)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('User declined the transaction @allureId(1908)', async ({
  wallet,
  app,
  widget,
}) => {
