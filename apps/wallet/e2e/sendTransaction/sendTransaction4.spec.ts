...
test('[messages] Error if contains invalid message @allureId(2243)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[messages] Success if contains maximum messages @allureId(1959)', async ({
  wallet,
  app,
  widget,
}) => {
...
test("[network] Success if '-239' (mainnet) @allureId(2249)", async ({
  wallet,
  app,
  widget,
}) => {
...
test('[payload] Error if invalid value @allureId(2246)', async ({
  wallet,
  app,
  widget,
}) => {
...
test('[payload] Success if absent @allureId(2228)', async ({
  wallet,
  app,
  widget,
}) => {
