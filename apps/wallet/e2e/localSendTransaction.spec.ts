...
test.describe('Local Send Transaction', () => {
  test('should trigger transaction request callback when sending TON locally', async ({
    wallet,
  }) => {
  ...
  test('should approve local TON transaction successfully', async ({
    wallet,
  }) => {
    await suite('Local Send Transaction');
    await label('sub-suite', 'handleNewTransaction fix');
    ...
    await expect(app.getByTestId('wallet-menu')).toBeVisible({
      timeout: 10000,
    });
