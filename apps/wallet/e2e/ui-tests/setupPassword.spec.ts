...
test.describe('SetupPassword', () => {
  ...
  test.describe('display', () => {
    ...
    test('helper text is visible', async ({ setupPassword }) => {
      await step(
        'Verify the "remember your password" helper is visible',
        async () => {
    ...
    test('password fields have type password', async ({ setupPassword }) => {
      await step('Verify password input has type="password"', async () => {
        await expect(setupPassword.passwordInput).toHaveAttribute(
          'type',
          'password',
        );
      });
      await step('Verify confirm input has type="password"', async () => {
        await expect(setupPassword.confirmInput).toHaveAttribute(
          'type',
          'password',
        );
      });
  ...
  test.describe('disabled state', () => {
    test('button disabled when only password is filled', async ({
      setupPassword,
    }) => {
    ...
    test('button disabled when only confirm is filled', async ({
      setupPassword,
    }) => {
  ...
  test.describe('validation errors', () => {
    test('error when password is less than 4 characters', async ({
      setupPassword,
      page,
    }) => {
      ...
      await step(
        'Verify error contains "Password must be at least 4 characters"',
        async () => {
          await expect(
            page.getByText('Password must be at least 4 characters'),
          ).toBeVisible();
        },
      );
    });
  ...
  test.describe('positive', () => {
    test('valid password redirects to the recovery-phrase screen', async ({
      ...
      await step(
        'Verify the recovery-phrase (create-wallet) screen is displayed',
        async () => {
  ...
  test.describe('localStorage', () => {
    test('auth state is persisted correctly after submit', async ({
      setupPassword,
      setupWallet,
    }) => {
      await step('Submit valid password', async () => {
      ...
      await step(
        'Verify auth.isPasswordSet is true in localStorage',
        async () => {
          expect(store.state.auth.isPasswordSet).toBe(true);
        },
      );
      await step(
        'Verify auth.passwordHash is saved in localStorage',
        async () => {
      ...
      await step(
        'Verify auth.currentPassword is not persisted in localStorage',
        async () => {
  ...
  test.describe('edge cases', () => {
    test('very long password (500 chars) is accepted', async ({
      setupPassword,
      setupWallet,
    }) => {
    ...
    test('pasted password is validated correctly', async ({
      setupPassword,
      setupWallet,
      context,
    }) => {
    ...
    test('XSS attempt in password field is treated as plain text', async ({
      setupPassword,
      setupWallet,
      page,
    }) => {
