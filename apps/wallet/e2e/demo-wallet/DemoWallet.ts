...
export class DemoWallet extends WalletApp {
  ...
  async importWallet(mnemonic: string): Promise<void> {
    if (mnemonic === '') {
      throw new Error(
        '[importWallet] mnemonic is required setup WALLET_MNEMONIC',
      );
    }
  ...
  async connectBy(
    url: string,
    shouldSkipConnect: boolean = false,
  ...
  async connect(
    ...
    const chose = app.getByTestId(
      confirm ? 'connect-approve' : 'connect-reject',
    );
  ...
  async signData(confirm: boolean = true): Promise<void> {
    ...
    await modal.waitFor({ state: 'visible' });
    const chose = app.getByTestId(
      confirm ? 'sign-data-approve' : 'sign-data-reject',
    );
    await chose.waitFor({ state: 'visible' });
    await chose.click();
    await modal.waitFor({ state: 'detached' });
  ...
  async accept(confirm: boolean = true): Promise<void> {
    ...
    await modal.waitFor({ state: 'visible' });
    const chose = app.getByTestId(
      confirm ? 'send-transaction-approve' : 'send-transaction-reject',
    );
    await chose.waitFor({ state: 'visible' });
    await chose.click();
    await modal.waitFor({ state: 'detached' });
