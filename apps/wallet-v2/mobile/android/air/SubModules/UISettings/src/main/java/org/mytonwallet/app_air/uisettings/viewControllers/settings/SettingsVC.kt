...
class SettingsVC(context: Context) :
            ...
            SettingsItem.Identifier.ADD_ACCOUNT -> {
                ...
                nav.setRoot(
                    WalletContextManager.delegate?.get()
                        ?.getAddAccountVC(WGlobalStorage.getAddWalletNetwork()) as WViewController
                )
                window?.present(nav)
            }
    ...
    private fun computeVisibleSections(): List<SettingsSection> {
        val sections = settingsVM.settingsSections.filter {
            it.section != SettingsSection.Section.PORTFOLIO &&
                it.section != SettingsSection.Section.HELP &&
                it.section != SettingsSection.Section.ABOUT
        }
