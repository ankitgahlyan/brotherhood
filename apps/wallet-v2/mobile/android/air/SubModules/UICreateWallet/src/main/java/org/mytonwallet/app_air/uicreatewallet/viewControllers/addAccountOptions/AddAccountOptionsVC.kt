...
class AddAccountOptionsVC(
    context: Context,
    network: MBlockchainNetwork = WGlobalStorage.getAddWalletNetwork(),
    val isOnIntro: Boolean
) : WViewController(context) {
    @Suppress("PropertyName")
    override val TAG = "AddAccountOptions"

    var network: MBlockchainNetwork = network
        private set
    ...
    private val networkSwitcher: WSegmentedControlGroup by lazy {
        WSegmentedControlGroup(context).apply {
            listOf(
                LocaleController.getString("Testnet"),
                LocaleController.getString("Mainnet")
            ).forEach { title ->
                addView(
            ...
            setSelectedIndex(if (network == MBlockchainNetwork.TESTNET) 0 else 1)
            setOnSelectedOptionChangeCallback { index ->
                val newNetwork = if (index ==
                    0
                ) {
    ...
    private fun onNetworkChanged(newNetwork: MBlockchainNetwork) {
        this.network = newNetwork
        WGlobalStorage.setAddWalletNetwork(newNetwork)
        setNavTitle(
            LocaleController.getString(if (showCreateButton) "Add Wallet" else "Import Wallet") +
                network.localizedIdentifier
        )
        updateSubWalletState()
    }

    private fun updateSubWalletState() {
        if (!showCreateSubWalletButton) return
        val activeNetwork = AccountStore.activeAccount?.network
        val isNetworkMatch = activeNetwork == network
        val description = if (isNetworkMatch) {
            LocaleController.getString("From current secret words")
        } else {
            LocaleController.getString("Only available for active wallet's network")
        }
        createSubWalletRow.configure(
            item = SettingsItem(
    ...
    private val scrollingContentView: WView by lazy {
        WView(context).apply {
            addView(networkSwitcher, FrameLayout.LayoutParams(WRAP_CONTENT, 38.dp))
            if (showCreateButton) {
            ...
            setConstraints {
                toTop(networkSwitcher, 14f)
                toCenterX(networkSwitcher)
                if (showCreateButton) {
                    topToBottom(createNewWalletView, networkSwitcher, 14f)
                    toCenterX(createNewWalletView)
                    topToBottom(secretWordsRow, createNewWalletView, 1f)
                } else {
                    topToBottom(secretWordsRow, networkSwitcher, 14f)
                }
    ...
    override fun updateTheme() {
        ...
        networkSwitcher.updateTheme()
        networkSwitcher.setBackgroundColor(WColor.ThumbBackground.color, 19f.dp)
    }
