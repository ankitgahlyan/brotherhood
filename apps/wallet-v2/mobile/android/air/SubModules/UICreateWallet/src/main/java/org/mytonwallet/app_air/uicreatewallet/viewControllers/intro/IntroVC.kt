...
class IntroVC(
    ...
    private fun applyContentConstraints() {
        ...
        contentView.setConstraints {
            ...
            bottomToTop(termsView, createNewWalletButton, 16f)
            toCenterX(termsView, 32f)
            bottomToTop(networkSwitcher, termsView, 16f)
            toCenterX(networkSwitcher)
        }

        val minRequiredHeight = 634.dp + (navigationController?.getSystemBars()?.top ?: 0) -
            (navigationController?.getSystemBars()?.bottom ?: 0)
