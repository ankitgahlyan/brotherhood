...
if [ "$IS_STATOSCOPE" != "1" ]; then
    FILES_TO_REMOVE+=("statoscope-build-statistics.json" "statoscope-report.html")
fi
...
if [ "$IS_TELEGRAM_APP" = "1" ] && [ "$DESTINATION" = "dist" ]; then
    ...
    if [ ! -f "$HEADERS_FILE" ] && [ -f "$DESTINATION/_headers_telegram" ]; then
        cp "$DESTINATION/_headers_telegram" "$HEADERS_FILE"
    fi
...
if [ -d "./functions" ] && [ "$IS_EXTENSION" != "1" ] && [ "$IS_PACKAGED_ELECTRON" != "1" ]; then
    cp -R ./functions "$DESTINATION/"
fi
