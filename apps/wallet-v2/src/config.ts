...
export const MNEMONIC_COUNTS = [24];

// Feature toggles
export const IS_AGENT_ENABLED = false;
export const IS_MARKET_ENABLED = false;
export const IS_EXPLORE_ENABLED = false;
export const IS_PORTFOLIO_ENABLED = false;
export const IS_MULTISEND_ENABLED = false;
export const IS_HELP_AND_SUPPORT_ENABLED = false;
export const IS_ABOUT_ENABLED = false;
...
const devPort = Number(process.env.PORT) || 1235;
export const USE_API_PROXY = process.env.USE_API_PROXY === '1';

function resolveProxyUrl(path: string, devProxyUrl: string, fallbackUrl: string): string {
  if (USE_API_PROXY) {
  ...
  if (APP_ENV === 'development' && !IS_EXTENSION && !IS_PACKAGED_ELECTRON) {
    return devProxyUrl;
  }
  return fallbackUrl;
}

export const TONCENTER_MAINNET_URL = (
  resolveProxyUrl(
    '/toncenter-proxy',
    `http://localhost:${devPort}/toncenter-proxy`,
...
export const TONAPIIO_MAINNET_URL = (
  resolveProxyUrl(
    '/tonapiio-proxy',
    `http://localhost:${devPort}/tonapiio-proxy`,
    process.env.TONAPIIO_MAINNET_URL || 'https://tonapiio.mytonwallet.org',
  )
);

export const TONCENTER_TESTNET_URL = (
  resolveProxyUrl(
    '/toncenter-testnet-proxy',
    `http://localhost:${devPort}/toncenter-testnet-proxy`,
...
export const TONAPIIO_TESTNET_URL = (
  resolveProxyUrl(
    '/tonapiio-testnet-proxy',
    `http://localhost:${devPort}/tonapiio-testnet-proxy`,
    process.env.TONAPIIO_TESTNET_URL || 'https://tonapiio-testnet.mytonwallet.org',
  )
);
export const IS_STREAMING_SOCKET_ENABLED = false;

export const BRILLIANT_API_BASE_URL = process.env.BRILLIANT_API_BASE_URL
  || resolveProxyUrl('/api-backend', `http://localhost:${devPort}/api-backend`, 'https://api.mywallet.io');
export const PROXY_API_BASE_URL = process.env.PROXY_API_BASE_URL
  || resolveProxyUrl('/api-backend/proxy', `http://localhost:${devPort}/api-backend/proxy`, 'https://api.mywallet.io/proxy');
...
export const TRON_MAINNET_API_URL = (
  APP_ENV === 'development' && !IS_EXTENSION && !IS_PACKAGED_ELECTRON
  && (!process.env.TRON_MAINNET_API_URL || process.env.TRON_MAINNET_API_URL === 'https://tronapi.mytonwallet.org')
    ? `http://localhost:${devPort}/tron-proxy`
    : process.env.TRON_MAINNET_API_URL || 'https://tronapi.mytonwallet.org'
);
