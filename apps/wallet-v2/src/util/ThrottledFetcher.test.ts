...
describe('ThrottledFetcher', () => {
  ...
  it('throttles direct public endpoints without API keys to 1 request per second', async () => {
    const fetchMock = global.fetch as jest.Mock;
    fetchMock.mockResolvedValue({ ok: true } as Response);

    const directUrl = 'https://testnet.toncenter.com/api/v3/transactions';

    // First request executes immediately
    await fetchWithThrottledProvider(directUrl);
    expect(fetchMock).toHaveBeenCalledTimes(1);

    // Second request without keys is queued for 1000ms
    const secondPromise = fetchWithThrottledProvider(directUrl);
    await Promise.resolve();
  ...
  it('allows faster 250ms spacing when an API key is provided for direct endpoints', async () => {
    const fetchMock = global.fetch as jest.Mock;
    fetchMock.mockResolvedValue({ ok: true } as Response);

    const directUrl = 'https://testnet.toncenter.com/api/v3/transactions';
    const init = { headers: { 'X-Api-Key': 'my-secret-key' } };

    await fetchWithThrottledProvider(directUrl, init);
    expect(fetchMock).toHaveBeenCalledTimes(1);

    const secondPromise = fetchWithThrottledProvider(directUrl, init);
    await Promise.resolve();
