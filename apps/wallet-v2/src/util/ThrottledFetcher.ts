...
export async function fetchWithThrottledProvider(
  ...
  const { fetcher, cacheKey } = getProviderFetcher(url.origin, init);
  const response = await fetcher.fetch(input, init, timeoutMs);
  adjustProviderDelay(cacheKey, response);
  return response;
}
...
const DIRECT_PUBLIC_PROVIDER_ORIGINS = new Set([
  'https://testnet.toncenter.com',
  'https://testnet.tonapi.io',
  'https://toncenter.com',
  'https://tonapi.io',
...
function getProviderFetcher(origin: string, init?: RequestInit) {
  let isDirectWithoutKey = false;
  if (DIRECT_PUBLIC_PROVIDER_ORIGINS.has(origin)) {
    let hasKey = false;
    if (init?.headers) {
      const headers = new Headers(init.headers as any);
      hasKey = Boolean(headers.get('X-Api-Key') || headers.get('Authorization'));
    }
    isDirectWithoutKey = !hasKey;
  }

  const minDelayMs = isDirectWithoutKey ? 1000 : PROVIDER_MIN_DELAY_MS;
  const cacheKey = `${origin}:${minDelayMs}`;
  let fetcher = throttledFetchers.get(cacheKey);
  if (!fetcher) {
    fetcher = new ThrottledFetcher(minDelayMs);
    throttledFetchers.set(cacheKey, fetcher);
  }

  return { fetcher, cacheKey };
}

function adjustProviderDelay(cacheKey: string, response: Response) {
  if (response.status !== 429) {
    return;
  }

  const retryAfterMs = getRetryAfterMs(response.headers) ?? PROVIDER_FALLBACK_RETRY_AFTER_MS;
  const fetcher = throttledFetchers.get(cacheKey);
  if (!fetcher) {
    return;
  }
