...
export function setupParticles(
  ...
  try {
    ...
    if (!manager) {
      const newManager = createParticleSystemManager(canvas);
      if (!newManager) {
        logDebug('setupParticles: WebGL is not supported in this environment');
        return () => {};
      }
      manager = newManager;
    ...
    logDebugError('setupParticles', error);
    return () => {};
  }
}

function createParticleSystemManager(canvas: HTMLCanvasElement): ParticleSystemManager | undefined {
  let maybeGl: WebGLRenderingContext | undefined;
  try {
    maybeGl = canvas.getContext('webgl', {
      alpha: true,
      antialias: false,
