...
import { IS_GRAM_WALLET, NO_EXTRA_FEATURES, POPULAR_WALLET_VERSIONS, TONCOIN } from '../../../config';
...
function setupStakingPolling(accountId: string, getBalances: () => Promise<ApiBalanceBySlug>, onUpdate: OnApiUpdate) {
  if (NO_EXTRA_FEATURES || IS_GRAM_WALLET || parseAccountId(accountId).network !== 'mainnet') {
    return () => {};
  }
