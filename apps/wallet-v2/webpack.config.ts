...
const canUseStatoscope = isStatoscopeBuild || Boolean(process.env.STATOSCOPE);
...
const cspConnectSrcHosts = Array.from(new Set([
  BRILLIANT_API_BASE_URL.startsWith('http') ? BRILLIANT_API_BASE_URL : undefined,
  BRILLIANT_API_BASE_URL.startsWith('http') ? BRILLIANT_API_BASE_URL.replace(/^http(s?):/, 'ws$1:') : undefined,
  PROXY_API_BASE_URL.startsWith('http') ? ensureTrailingSlash(PROXY_API_BASE_URL) : undefined,
  MW_STATIC_BASE_URL,
  TONCENTER_MAINNET_URL.startsWith('http') ? TONCENTER_MAINNET_URL : undefined,
  TONCENTER_MAINNET_URL.startsWith('http') ? TONCENTER_MAINNET_URL.replace(/^http(s?):/, 'ws$1:') : undefined,
  TONCENTER_TESTNET_URL.startsWith('http') ? TONCENTER_TESTNET_URL : undefined,
  TONCENTER_TESTNET_URL.startsWith('http') ? TONCENTER_TESTNET_URL.replace(/^http(s?):/, 'ws$1:') : undefined,
  'https://toncenter.com',
  'https://testnet.toncenter.com',
  'https://tonapi.io',
  'https://testnet.tonapi.io',
  TONAPIIO_MAINNET_URL.startsWith('http') ? TONAPIIO_MAINNET_URL : undefined,
  TONAPIIO_TESTNET_URL.startsWith('http') ? TONAPIIO_TESTNET_URL : undefined,
  TRON_MAINNET_API_URL.startsWith('http') ? TRON_MAINNET_API_URL : undefined,
  TRON_TESTNET_API_URL.startsWith('http') ? TRON_TESTNET_API_URL : undefined,
  SOLANA_MAINNET_RPC_URL.startsWith('http') ? SOLANA_MAINNET_RPC_URL : undefined,
  SOLANA_MAINNET_RPC_URL.startsWith('http') ? SOLANA_MAINNET_RPC_URL.replace(/^http(s?):/, 'ws$1:') : undefined,
  SOLANA_TESTNET_RPC_URL.startsWith('http') ? SOLANA_TESTNET_RPC_URL : undefined,
  SOLANA_TESTNET_RPC_URL.startsWith('http') ? SOLANA_TESTNET_RPC_URL.replace(/^http(s?):/, 'ws$1:') : undefined,
  SOLANA_MAINNET_API_URL.startsWith('http') ? SOLANA_MAINNET_API_URL : undefined,
  SOLANA_TESTNET_API_URL.startsWith('http') ? SOLANA_TESTNET_API_URL : undefined,
  WALLET_CONNECT_BRIDGE_PATTERNS,
  ...WALLET_CONNECT_PAY_CONNECT_ORIGINS,
  AGENT_API_URL ? new URL(AGENT_API_URL).origin : undefined,
  EVM_MAINNET_RPC_URL.startsWith('http') ? EVM_MAINNET_RPC_URL : undefined,
  EVM_TESTNET_RPC_URL.startsWith('http') ? EVM_TESTNET_RPC_URL : undefined,
  EVM_MAINNET_RPC_URL.startsWith('http') ? EVM_MAINNET_RPC_URL.replace(/^http(s?):/, 'ws$1:') : undefined,
  EVM_TESTNET_RPC_URL.startsWith('http') ? EVM_TESTNET_RPC_URL.replace(/^http(s?):/, 'ws$1:') : undefined,
  ...
  return {
    ...
    devServer: {
      ...
      proxy: [
        ...
        {
          context: ['/toncenter-proxy'],
          target: 'https://toncenter.mytonwallet.org',
          pathRewrite: { '^/toncenter-proxy': '' },
          changeOrigin: true,
          ws: true,
          proxyTimeout: 60000,
          timeout: 60000,
        ...
        },
        {
          ...
          target: 'https://testnet.tonapi.io',
        ...
        {
          context: ['/tron-proxy', '/wallet', '/walletsolidity', '/walletextension'],
          target: 'https://tronapi.mytonwallet.org',
          pathRewrite: { '^/tron-proxy': '' },
          changeOrigin: true,
          proxyTimeout: 60000,
          timeout: 60000,
          headers: {
    ...
    resolve: {
      ...
      alias: {
        ...
        '@wrappers': path.join(__dirname, '../../wrappers-ts'),
